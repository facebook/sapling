// @generated SignedSource<<2dbc5fcfbf4e1ba6293ecddd5b0c1182>>
...
//
// Source: https://www.internalfb.com/code/configerator/[master]/source/scm/mononoke/repos/repos.thrift
/*
...
//
//   4) If you wish to have a thrift-optional field "required" when config is built,
//      enforce it in config validator in repos.thrift-cvalidator.
...
// Hook settings that can be used to override the default hook configuration
// derived from metagit configs.
struct RawHookSettings {
  // List of hook configurations to use for the repo
  1: list<RawHookConfig> hook_configs;
  // Whether to enforce fast-forward-only pushes on bookmarks
  2: bool fast_forward_only;
}
...
struct QuickRepoDefinition {
  ...
  // Temporary gating to enable this derived data type for a subset of repos
  // TODO(lyang) Delete once everything is backfilled and enabled
  17: optional bool enable_inferred_copy_from_derived_data;
  // Whether to disable all hooks for this repo. Relevant in cases the Git repo is essentially a mirror
  // 18: optional bool disable_all_hooks --> Deleted since it was never used
  19: optional bool enable_cgdm;
  // Whether to enable shallow sharding for this repo in SCS. Relevant for x-repo-sync repos
  20: optional bool shallow_shard_in_scs;
  // Custom bookmarks configs for this repo. Useful for having custom branch level ACLs, different hooks, etc.
  21: optional list<RawBookmarkConfig> custom_bookmarks_configs;
  // Override hook settings derived from metagit configs. When set, these settings
  // will be used instead of the default hook configuration from metagit.
  22: optional RawHookSettings override_hook_settings;
  23: optional bool enable_gdm_v3;
  // Whether to skip automatic sharding setup for this repo.
  // When set, sharding must be configured manually in sharding.cinc.
  24: optional bool skip_sharding;
  // Whether to enable derivation for content manifests
  25: optional bool enable_content_manifests;
}
...
// T-shirt size for sharding allocation. Used by RepoSpec.
// Values match QuickRepoDefinitionTShirtSize for migration compatibility.
enum TShirtSize {
  SMALL = 0,
  MEDIUM = 1,
  LARGE = 2,
...
// Controls how repo is allocated to server jobs. Used by RepoSpec.
// Values match QuickRepoDefinitionShardingConfig for migration compatibility.
enum ShardingRegions {
  ALL_REGIONS = 0,
  BGM_ONLY_REGIONS = 1,
}

// A self-contained repo specification combining identity, tier membership,
// sharding, and runtime config into a single declaration.
// Each Git repo gets its own .cconf file exporting one RepoSpec.
//
// NOTE: Never consumed directly by Mononoke at runtime. Only used to generate
// materialized configs (per-tier RawRepoConfig JSON, TierManifest, ShardManager config).
struct RepoSpec {
  // Identity (from current RawRepoDefinition)
  1: i32 repo_id;
  2: string repo_name;
  ...
  5: bool readonly;
  6: RawCommitIdentityScheme default_commit_identity_scheme;
  7: optional bool enable_git_bundle_uri;

  // Tier + Sharding
  8: list<string> tiers;
  9: TShirtSize t_shirt_size;
  10: ShardingRegions sharding_regions;

  // Runtime config (embeds existing RawRepoConfig — sparse, only set fields)
  11: optional RawRepoConfig repo_config;

  // Per-tier overrides (partial RawRepoConfig merged on top of repo_config per tier)
  12: optional map_string_RawRepoConfig_tier_overrides tier_overrides;
}

// Lightweight manifest listing all repos in a tier.
// Loaded by MononokeConfigs as a single ConfigHandle.
struct TierManifest {
  1: list<TierRepoEntry> repos;
  2: RawCommonConfig common;
  3: map_string_RawStorageConfig_2830 storage;
}

// Entry in a TierManifest for one repo.
struct TierRepoEntry {
  1: string repo_name;
  2: i32 repo_id;
  // Configerator path to the per-tier-per-repo RawRepoConfig JSON
  3: string config_path;
  // Whether this repo uses deep sharding (if true, config is only loaded
  // when ShardManager assigns the repo to this task)
  4: bool is_deep_sharded;
}
...
struct RawCommitRateLimit {
  1: i64 window_secs;
  2: i64 max_commits;
}

struct RawAlwaysPassEligibilityCheck {}

union RawEligibilityCheck {
  1: string commit_message_tag;
  2: string hg_extra_key;
  3: RawAlwaysPassEligibilityCheck always_pass;
}

struct RawCommitRateLimitRule {
  1: string name;
  2: list<RawEligibilityCheck> eligibility_checks;
  3: list<RawCommitRateLimit> limits;
  4: list<string> directories;
  5: bool per_user;
  // TODO(T259128745): maybe add a purpose field or custom msg that can be used
  // to inform the user why the rate limit exists (e.g. what it covers, etc).
}

struct RawCommitRateLimitCacheConfig {
  1: i64 max_entries;
  2: i64 ttl_secs;
}

struct RawCommitRateLimitConfig {
  1: list<RawCommitRateLimitRule> rules;
  2: optional RawCommitRateLimitCacheConfig cache_config;
}
...
struct RawRepoConfig {
  ...
  71: optional RawDirectoryBranchClusterConfig directory_branch_cluster_config;
  // Configuration for the repo's restricted paths
  72: optional RawRestrictedPathsConfig restricted_paths_config;
  // Configuration for remote diff service
  73: optional RawRemoteDiffConfig remote_diff_config;
  // Configuration for commit rate limiting
  74: optional RawCommitRateLimitConfig commit_rate_limit_config;
  // Configuration for connecting to Zelos for the derivation pipeline
  75: optional RawZelosConfig pipeline_zelos_config;
  // When set, reads against this repo's blobstore fall through to the named
  // repo's blobstore on miss. Writes always go to this repo only. Intended
  // for perf-test mirror repos that share backing storage with a source repo
  // (e.g. fbsource_mirror -> fbsource) so the mirror can reuse production
  // blobs without copying them.
  76: optional i32 read_fallback_repo_id;
  // When true, an upload to this repo may bypass redaction (log-only, not
  // blocked) if the caller holds the mirror_upload permission. Defaults to
  // false: redaction is enforced.
  77: optional bool mirror_upload_redaction_bypass_enabled;
}
...
struct RawGitConfigs {
  ...
  // Blobstore key for preloading CGDM blob
  5: optional string preloaded_cgdm_blobstore_key;
}
...
struct RawDerivedDataConfig {
  ...
  // 13, 14: deleted — do not reuse. The legacy `pipeline_config` field name
  //         is also burned: configerator distributes JSON keyed by field
  //         name, so old binaries would try to parse the new shape as the
  //         old one and crash on the unset `type_config` union. The new
  //         field uses `pipeline_config_v2` so their JSON deserializer
  //         drops it as unknown.
  /// Unified, repo-level derivation pipeline configuration.
  15: optional RawDerivationPipelineConfig pipeline_config_v2;
  /// Scuba table for logging every bonsai<->hg mapping write, so writes are
  /// attributable independently of the calling service's scuba table.
  16: optional string bonsai_hg_mapping_scuba_table;
  /// Types the warm bookmarks cache must not wait on. Does not affect derivation.
  17: optional set<string> wbc_excluded_types;
}
...
struct RawDerivedDataTypesConfig {
  ...
  /// Maps derived data type name to XDB shard ID.
  /// Types present in this map use the XDB commit_derived_data table.
  /// Types absent continue to use the blobstore for mapping storage.
  14: optional map<string, i32> xdb_mapping_shard_ids;
}
...
@rust.Exhaustive
struct RawDerivationPipelineStageConfig {
  /// Absolute paths of the stages this one directly depends on.
  1: list<string> dependencies;
}

@rust.Exhaustive
struct RawDerivationPipelineConfig {
  /// Derivable types (by name) managed by this pipeline.
  1: set<string> types;
  /// Bookmarks to tail for this pipeline.
  2: list<string> bookmarks;
  /// Stage DAG, keyed by the stage's absolute path (`""` is the terminal
  /// stage at the repo root). Must form a DAG.
  3: map<string, RawDerivationPipelineStageConfig> stages;
  /// Pipeline-level batch size.
  4: i64 batch_size;
}
...
struct RawRemoteMetadataConfig {
  1: RawDbRemote primary; # DEPRECATED
  ...
  10: optional RawDbRemote restricted_paths;
  11: optional RawDbRemote production;
  12: optional RawDbRemote commit_graph;
  13: optional RawDbRemote bookmarks;
  14: optional RawDbRemote bonsai_hg_mapping;
  15: optional RawDbShardableRemote derived_data_mapping;
  16: optional RawDbRemote repo_manifest_mapping;
}

@rust.Exhaustive
struct RawOssRemoteMetadataConfig {
  1: RawOssDbRemote primary; # DEPRECATED
  ...
  11: optional RawOssDbRemote production;
  12: optional RawOssDbRemote commit_graph;
  13: optional RawOssDbRemote bookmarks;
  14: optional RawOssDbRemote bonsai_hg_mapping;
  15: optional RawOssDbRemote derived_data_mapping;
}
...
struct RawPushrebaseParams {
  ...
  // Conflicts on files under these prefixes will be rejected (not
  // auto-merged), preserving downstream signals like Hack type-checking.
  16: optional list<string> merge_resolution_excluded_path_prefixes;
  ...
  17: optional list<string> pessimistic_locking_bookmarks;
}

@rust.Exhaustive
struct RawBookmarkConfig {
  // Exactly one of the regex, inverse regex or the name should be provided
  1: optional string regex;
  2: optional string name;
  12: optional string inverse_regex;
...
struct RawCommonConfig {
  ...
  // Repo name prefix for RL Land Service push diversion. When set, pushes
  // to repos whose name starts with this prefix will be diverted to the
  // RL Land Service for atomic cross-repo bookmark movement.
  14: optional string rl_land_service_repo_prefix;

  // Configerator path to the MononokeRateLimits config this tier should
  // load. Defaults to `scm/mononoke/ratelimiting/ratelimits` when unset.
  // Use a sibling path (e.g. `scm/mononoke/ratelimiting/ratelimits_volatile`)
  // to give a tier its own load-shedding metrics and rate limits.
  15: optional string rate_limit_config_path;

  // Routes diverted pushes to their manifest repo for submit_manifest_land:
  // repo-name marker (substring, like rl_land_service_repo_prefix) -> the
  // manifest repo to name in the request. A repo appearing as a VALUE is a
  // manifest repo; pushes to it land as user manifest edits.
  16: optional map<string, string> multi_repo_land_manifest_repos;
}
...
struct RawHookConfig {
  ...
  /// AMP group name. When set, only members of this group can use the
  /// bypass_commit_string or bypass_pushvar to bypass this hook.
  13: optional string bypass_permission_group;
...
struct RawServiceWriteRestrictions {
  ...
  5: optional bool permit_create_commit_check_bypass;
}
...
@rust.Exhaustive
struct RawDirectoryBranchClusterFixedCluster {
  // Cluster primary path
  1: string cluster_primary;
  // Secondary paths
  2: list<string> secondaries;
}

@rust.Exhaustive
struct RawDirectoryBranchClusterFixedConfig {
  // List of clusters
  1: list<RawDirectoryBranchClusterFixedCluster> clusters;
}

@rust.Exhaustive
struct RawDirectoryBranchClusterConfig {
  // Fixed config for directory branch clusters
  1: RawDirectoryBranchClusterFixedConfig fixed_config;
}
...
@rust.Exhaustive
struct RawEnforcementConditionSet {
  ...
  1: optional bool always_enabled;
  /// Client entry points that trigger enforcement (e.g. "ScsServer", "EdenApi").
  /// Empty = don't filter on this dimension.
  2: optional list<string> entry_points;
  /** @deprecated Replaced by `restriction_acls` in field 5. */
  3: optional list<string> restriction_roots;
  /// If true, client must also send server_side_tenting=true in request metadata.
  4: optional bool require_client_request_flag;
  /**
   * ACL-identity scoping. Empty = match any restricted access.
   * Replaces `restriction_roots` (now @deprecated; runtime parser ignores it).
   */
  5: optional list<string> restriction_acls;
  /// Machine tiers (MACHINE_TIER identity values) that trigger enforcement.
  /// Empty = don't filter on this dimension. Match = caller's MACHINE_TIER
  /// identity value appears in this list.
  6: optional list<string> machine_tiers;
  /// Server build rules (the running binary's `build_info` build rule, i.e. the
  /// Buck target it was built from) that trigger enforcement. Empty = don't
  /// filter on this dimension. Match = the server's own build_rule (the value
  /// `add_common_server_data` logs to scuba) appears in this list.
  8: optional list<string> build_rules;

  /// Regexes evaluated against each caller identity's `"TYPE:value"` Display form.
  /// Empty = don't filter on this dimension. Match = any caller identity matches
  /// any regex on the list. Substring semantics — anchor with `^`/`$` for strict
  /// matching. Invalid regexes are rejected at config parse time.
  9: optional list<string> client_identity_regexes;
  /// Tri-state filter on whether the caller is an agent (its identities carry
  /// an `agent` attribute, see `MononokeIdentitySet::likely_an_agent`). Unset =
  /// don't filter on this dimension; true = match only agents; false = match
  /// only non-agents.
  10: optional bool is_agent;
}

@rust.Exhaustive
struct RawRestrictedPathsManifestIdStoreConfig {
  /// Whether to cache manifest-id entries in memory.
  1: optional bool use_manifest_id_cache;
  /// Interval between cache update attempts.
  2: optional i64 cache_update_interval_ms;
  /// Whether cache update attempts query only recently inserted rows.
  3: optional bool use_incremental_cache_updates;
  /// Number of IDs below the watermark to include in incremental queries.
  4: optional i64 incremental_cache_update_lookback_ids;
  /// Interval between full cache reconciliations when incremental updates are enabled.
  5: optional i64 cache_full_refresh_interval_ms;
}

@rust.Exhaustive
struct RawRestrictedPathsConfig {
  /**
   * @deprecated Not read by Mononoke. Use `path_restriction_metadata` (field 14).
   * The field is required, so it cannot be removed; every config sets it empty.
   * Adding an entry here has no effect and will not restrict a path.
   */
  1: map<string, string> path_acls;
  /** @deprecated Use `manifest_id_store_config` instead. */
  2: optional bool use_manifest_id_cache;
  /** @deprecated Use `manifest_id_store_config` instead. */
  3: optional i64 cache_update_interval_ms;
  4: optional list<RawSoftRestrictedPathConfig> soft_path_acls;
  # DEPRECATED: use `enforcement_condition_sets` instead
  5: optional list<RawEnforcementCondition> enforcement_conditions; # DEPRECATED
  // Optional ACL for tooling that should be allowlisted for all restricted paths.
  // If set, callers matching this ACL will be logged as allowlisted tooling.
  6: optional string tooling_allowlist_acl;
  # DEPRECATED: use `enforcement_condition_sets` instead
  7: optional list<string> conditional_enforcement_acls; # DEPRECATED
  /// Name of the ACL files (default: ".slacl")
  8: optional string acl_file_name;
  /**
   * @deprecated Not read by Mononoke. Use the per-tent `rollout_allowlist_group`
   * on `RawPathRestrictionMetadata` / `RawRestrictedPathsAclFile` instead.
   * A repo-wide allowlist forces every tent to trust the same identities; the
   * per-tent field lets each tent owner hold and triage their own.
   */
  9: optional string rollout_allowlist_acl;
  /// Condition sets for enforcement. OR across sets, AND within.
  /// Replaces conditional_enforcement_acls (field 7).
  10: optional list<RawEnforcementConditionSet> enforcement_condition_sets;
  /**
   * Master kill switch for path ACL enforcement on this repo.
   * Replaces the `enable_server_side_path_acls` JK.
   * Optional during the migration window — runtime treats absent as `false`.
   */
  11: optional bool enforcement_enabled;
  /**
   * 4-stage rollout state for AclManifest as the source of truth for
   * restricted-path enforcement. Replaces the
   * `use_acl_manifest_for_restricted_paths` JK.
   * Values: "Disabled", "Shadow", "Both", "Authoritative".
   * Defaults to "Disabled" when absent.
   */
  12: optional string acl_manifest_mode;
  /// AMP group that can be used by admins to temporarily bypass all Path ACLs
  /// enforcement when fighting SEVs or debugging issues.
  13: optional string admin_bypass_group;
  // Per-path restriction metadata. The sole source of config-backed path
  // restrictions; the legacy `path_acls` map (field 1) is deprecated and empty.
  14: optional map<
    string,
    RawPathRestrictionMetadata
  > path_restriction_metadata;
  15: optional list<string> manifest_id_denylist; # DEPRECATED
  /// Manifest-id store cache configuration.
  16: optional RawRestrictedPathsManifestIdStoreConfig manifest_id_store_config;
  /// Free text appended to the error returned to callers denied access to a
  /// restricted path, e.g. a pointer to a rollout announcement or an active
  /// SEV. Absent = no extra text.
  17: optional string denial_message;
}

# DEPRECATED: use `enforcement_condition_sets` instead
@rust.Exhaustive
struct RawEnforcementCondition {
  # DEPRECATED: use `enforcement_condition_sets` instead
  1: optional list<string> client_identities;
  // DEPRECATED: only use client_identities
  2: optional string sandcastle_alias; # DEPRECATED
  // DEPRECATED: only use client_identities
  3: optional string client_main_id; # DEPRECATED
}

@rust.Exhaustive
struct RawSoftRestrictedPathConfig {
  1: string path;
  2: string acl;
  3: i64 max_copied_files_limit;
}

// Restriction metadata for a single restricted path: the repo-region ACL, an
// optional permission-request group, and a `read_only` flag that, when true,
// stops new manifest-id-store entries being derived for the path (enforcement
// on existing entries and config-based authorization are unaffected).
@rust.Exhaustive
struct RawPathRestrictionMetadata {
  1: string repo_region_acl;
  2: optional string permission_request_group;
  3: optional bool read_only;
  /// Bare AMP group name (no `GROUP:` prefix) whose members are temporarily
  /// allowlisted to read this path during rollout. The tent owner triages the
  /// members and either grants them access in the repo region ACL or removes
  /// them. A caller must be allowlisted for *every* restricted path in a
  /// request for the allowlist to grant access.
  4: optional string rollout_allowlist_group;
}

/// Contents of a .slacl file defining path restrictions
@rust.Exhaustive
struct RawRestrictedPathsAclFile {
  1: optional i32 version;
  /// REPO_REGION ACL protecting this directory
  /// e.x. "repos/hg/fbsource/=project1"
  2: string repo_region_acl;
  ...
  /// Bare AMP group name (no `GROUP:` prefix) whose members are temporarily
  /// allowlisted to read this directory during rollout. The tent owner triages
  /// the members and either grants them access in the repo region ACL or
  /// removes them. A caller must be allowlisted for *every* restricted path in
  /// a request for the allowlist to grant access.
  4: optional string rollout_allowlist_group;
...
@rust.Type{name = "HashMap"}
typedef map<string, RawRepoConfig> map_string_RawRepoConfig_tier_overrides
