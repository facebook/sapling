// @generated SignedSource<<c8197d806f1ed2cdd6c975f2c2dea569>>
...
struct RawRestrictedPathsConfig {
  // Map of restricted paths to their corresponding ACLs
  ...
  // Optional ACL for tooling that is allowlisted during rollout of restricted paths.
  // Tooling in this group will either be permanently allowlisted or blocked,
  // depending on whether they provide the needed safeguards for tented code
  // and gracefully handle unauthorized access errors.
  ...
  // Per-path restriction metadata. When present for a path, supersedes that
  // path's `path_acls` entry. During migration both may be set; an entry here
  // wins on matching ACL and is rejected on divergence by the consumer.
  // TODO(T277178795): update this comment and deprecate `path_acls` after the migration.
  14: optional map<
    string,
...
// Restriction metadata for a single restricted path. Supersedes the bare
// path -> ACL entry in `path_acls`: carries the repo-region ACL, an optional
// permission-request group, and a `read_only` flag that, when true, stops new
// manifest-id-store entries being derived for the path (enforcement on existing
// entries and config-based authorization are unaffected).
@rust.Exhaustive
