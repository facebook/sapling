// @generated SignedSource<<7bc12a4d89068d1e8c08ee447926c641>>
...
struct RawDerivedDataConfig {
  ...
  /// Derivation pipeline configuration per derived data type name.
  13: optional map<string, RawDerivationPipelineTypeConfig> derivation_pipeline;
  /// Unified, repo-level derivation pipeline configuration. The parser will
  /// build a single in-memory `DerivationPipelineConfig` from this field. The
  /// per-type `derivation_pipeline` (field 13) remains for now; consumers will
  /// be migrated in follow-up steps.
  14: optional RawDerivationPipelineConfig pipeline_config;
}
...
@rust.Exhaustive
struct RawManifestDerivationPipelineConfig {
  /// The path prefix this stage is responsible for.
  1: string path;
}

union RawDerivationPipelineStageTypeConfig {
  1: RawManifestDerivationPipelineConfig manifest;
}

@rust.Exhaustive
struct RawDerivationPipelineStageConfig {
  /// Stage IDs this stage depends on (direct dependencies only).
  1: list<string> dependencies;
  /// Whether this is the terminal stage (exactly one per type).
  2: bool terminal;
  /// Type-specific stage configuration.
  3: RawDerivationPipelineStageTypeConfig type_config;
}

@rust.Exhaustive
struct RawDerivationPipelineTypeConfig {
  /// Bookmarks to use derivation pipeline for.
  1: list<string> bookmarks;
  /// Map from stage ID to stage config. Must form a DAG.
  2: map<string, RawDerivationPipelineStageConfig> stages;
}
...
struct RawDerivationPipelineConfig {
  ...
  /// Stage DAG, shared across all types in this pipeline.
