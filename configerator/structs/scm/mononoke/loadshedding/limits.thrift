// @generated SignedSource<<20776c66ca8e1ddac9c6a3181b1baaff>>
...
struct StaticSlicedLimit {
    // Limit will apply if identities are subset of user's identities.
    1: list<string> identities;
    // Will consistently limit limit_pct clients with specified subset of identities.
    2: i32 limit_pct;
}

struct StaticSlicedLimits {
    // First matching limit will apply.
    1: list<StaticSlicedLimit> limits;
    // Nonce to rotate slices.
    2: string nonce;
}

struct RateLimits {
  // How many pushes can a single author make
  1: RateLimit commits_per_author;
  // How many file changes can be caused by public commits in a given repo
  2: optional map<RepoId, RateLimit> public_file_changes;
  // How many file changes can be caused by infinitepush commits in a given repo
  3: optional map<RepoId, RateLimit> infinitepush_file_changes;
  // How many file changes we allow in total (combined across repos)
  5: optional RateLimit total_file_changes;
}
...
struct MononokeThrottleLimits {
  ...
  // Consistent percentage of load shedding tiers (specified as identity subsets).
  // Percentage is the the percentage of hosts retained from the tier.
  6: optional StaticSlicedLimits static_sliced_limits;
}
