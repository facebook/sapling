                                   ...
                                   const linelog_linenum *blinenums)
{
	...
	linelog_loffset newlen =
	    (linelog_loffset)oldlen +
...
#define appendinst(inst) mustsuccess(writeinst(buf, &inst, inst0.offset++));
	...
	if (a1 < a2) { /* [2] */
		...
		linelog_inst jge = {
		    .opcode = JGE, .rev = brev, .offset = a2addr};
		appendinst(jge);
	}
...
#undef appendinst
	...
	if (a2 - a1 != b2 - b1) {
		size_t movesize =
		    sizeof(linelog_lineinfo) * (ar->linecount + 1 - a2);
...
{
	return replacelines(buf, ar, brev, a1, a2, 0, blinecount, brevs,
	                    blinenums);
}
