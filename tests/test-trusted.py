...
def testui(
    ...
    print(
        "# %s user, %s group%s" % (kind[user == cuser], kind[group == cgroup], trusted)
    )
...
print(u.configint("foo", "int", 0), u.configint("foo", "int", 0, untrusted=True))
print("# bytes, trusted and untrusted")
print(
    u.configbytes("foo", "bytes", 0), u.configbytes("foo", "bytes", 0, untrusted=True)
)
print("# list, trusted and untrusted")
print(u.configlist("foo", "list", []), u.configlist("foo", "list", [], untrusted=True))
