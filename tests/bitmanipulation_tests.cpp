...
TEST(BitManipulation, getbe32)
{
	...
	EXPECT_EQ(0x12345678, getbe32(make_buf(0x12, 0x34, 0x56, 0x78).data()));
	EXPECT_EQ(0xffffffffUL,
	          getbe32(make_buf(0xff, 0xff, 0xff, 0xff).data()));
}
...
TEST(BitManipulation, getbefloat64)
{
	...
	EXPECT_EQ(-0.0,
	          getbefloat64(make_buf(0x80, 0, 0, 0, 0, 0, 0, 0).data()));
	EXPECT_DOUBLE_EQ(
	    2.0, getbefloat64(make_buf(0x40, 0, 0, 0, 0, 0, 0, 1).data()));
	EXPECT_DOUBLE_EQ(
	    -8.0, getbefloat64(make_buf(0xc0, 0x20, 0, 0, 0, 0, 0, 1).data()));
	EXPECT_DOUBLE_EQ(-4.0, getbefloat64(make_buf(0xc0, 0x0f, 0xff, 0xff,
	                                             0xff, 0xff, 0xff, 0xff)
	                                        .data()));
}
