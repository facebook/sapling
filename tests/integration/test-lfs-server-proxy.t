...
# Upload data to upstream only
  ...
  $ cat "$log_upstream"
  *POST /lfs_upstream/objects/batch * 200 * (glob)
  *PUT /lfs_upstream/upload/ab02c2a1923c8eb11cb3ddab70320746d71d32ad63f255698dc67c3295757746/2048 * 200 * (glob)
...
# Reading data should succeed if it is in upstream
  ...
  $ cat "$log_proxy"
  *POST /lfs_proxy/objects/batch * 200 * (glob)

  $ cat "$log_upstream"
  *POST /lfs_upstream/objects/batch * 200 * (glob)
  *GET /lfs_upstream/download/d28548bc21aabf04d143886d717d72375e3deecd0dafb3d110676b70a192cb5d * 200 * (glob)
...
# Uploading data that is present in upstream but not locally should trigger a new upload
  ...
  $ cat "$log_proxy"
  *POST /lfs_proxy/objects/batch * 200 * (glob)
  *PUT /lfs_proxy/upload/ab02c2a1923c8eb11cb3ddab70320746d71d32ad63f255698dc67c3295757746/2048 * 200 * (glob)
...
# Uploading should make data available in both locations
  ...
  $ cat "$log_proxy"
  *POST /lfs_proxy/objects/batch * 200 * (glob)
  *PUT /lfs_proxy/upload/a1bcf2c963bec9588aaa30bd33ef07873792e3ec241453b0d21635d1c4bbae84/2048 * 200 * (glob)

  $ cat "$log_upstream"
  *POST /lfs_upstream/objects/batch * 200 * (glob)
  *POST /lfs_upstream/objects/batch * 200 * (glob)
  *PUT /lfs_upstream/upload/a1bcf2c963bec9588aaa30bd33ef07873792e3ec241453b0d21635d1c4bbae84/2048 * 200 * (glob)
  ...
  $ cat "$log_proxy"
  *POST /lfs_proxy/objects/batch * 200 * (glob)
  *GET /lfs_proxy/download/2e8e6e2dda2bb7b6458146a1c1bf301e4856293e1cc258ab789c63df2254693c * 200 * (glob)

  $ cat "$log_upstream"
  *POST /lfs_upstream/objects/batch * 200 * (glob)
  *POST /lfs_upstream/objects/batch * 200 * (glob)
  *GET /lfs_upstream/download/2e8e6e2dda2bb7b6458146a1c1bf301e4856293e1cc258ab789c63df2254693c * 200 * (glob)
