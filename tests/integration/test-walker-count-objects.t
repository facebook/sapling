...
count-objects, bonsai core data.  total nodes is BONSAICOUNT plus one for the root bookmark step.
  ...
  Walking roots * (glob)
  Walking edge types [BonsaiChangesetToBonsaiParent, BonsaiChangesetToFileContent, BookmarkToBonsaiChangeset, FileContentToFileContentMetadata]
  Walking node types [BonsaiChangeset, Bookmark, FileContent, FileContentMetadata]
  Final count: (10, 10)
...
count-objects, shallow, bonsai only.  No parents, expect just one of each node type
  ...
  Walking roots * (glob)
  Walking edge types [BonsaiChangesetToFileContent, BookmarkToBonsaiChangeset, FileContentToFileContentMetadata]
  Walking node types [BonsaiChangeset, Bookmark, FileContent, FileContentMetadata]
  Final count: (4, 4)
...
count-objects, default deep walk across bonsai and hg data.  BLOBCOUNT plus mappings and root.
  ...
  Walking node types [BonsaiChangeset, BonsaiHgMapping, Bookmark, FileContent, FileContentMetadata, HgBonsaiMapping, HgChangeset, HgFileEnvelope, HgFileNode, HgManifest]
  Final count: (28, 28)
...
count-objects, default shallow walk across bonsai and hg data
  ...
  Walking roots * (glob)
  Walking edge types [BonsaiChangesetToBonsaiHgMapping, BonsaiChangesetToFileContent, BonsaiHgMappingToHgChangeset, BookmarkToBonsaiChangeset, FileContentToFileContentMetadata, HgChangesetToHgManifest, HgFileEnvelopeToFileContent, HgManifestToChildHgManifest, HgManifestToHgFileEnvelope, HgManifestToHgFileNode]
  Walking node types [BonsaiChangeset, BonsaiHgMapping, Bookmark, FileContent, FileContentMetadata, HgChangeset, HgFileEnvelope, HgFileNode, HgManifest]
  Final count: (17, 17)
