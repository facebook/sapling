...
count-objects, bonsai core data.  total nodes is BONSAICOUNT plus one for the root bookmark step.
  ...
  * Walking roots * (glob)
  * Walking edge types [BonsaiChangesetToBonsaiParent, BonsaiChangesetToFileContent, BookmarkToBonsaiChangeset, FileContentMetadataToGitSha1Alias, FileContentMetadataToSha1Alias, FileContentMetadataToSha256Alias, FileContentToFileContentMetadata] (glob)
  * Walking node types [BonsaiChangeset, FileContent, FileContentMetadata] (glob)
...
count-objects, shallow, bonsai only.  No parents, expect just one of each node type
  ...
  * Walking roots * (glob)
  * Walking edge types [BonsaiChangesetToBonsaiHgMapping, BonsaiChangesetToFileContent, BookmarkToBonsaiChangeset, FileContentMetadataToGitSha1Alias, FileContentMetadataToSha1Alias, FileContentMetadataToSha256Alias, FileContentToFileContentMetadata, HgBonsaiMappingToBonsaiChangeset] (glob)
  * Walking node types [BonsaiChangeset, FileContent, FileContentMetadata] (glob)
...
count-objects, hg only. total nodes is HGCOUNT plus 1 for the root bookmark step, plus 1 for mapping from bookmark to hg. plus 3 for filenode (same blob as envelope)
  ...
  * Walking roots * (glob)
  * Walking edge types [BonsaiHgMappingToHgChangeset, BookmarkToBonsaiHgMapping, HgChangesetToHgManifest, HgChangesetToHgParent, HgFileEnvelopeToFileContent, HgFileNodeToHgParentFileNode, HgLinkNodeToHgChangeset, HgManifestToChildHgManifest, HgManifestToHgFileEnvelope, HgManifestToHgFileNode] (glob)
  * Walking node types [BonsaiHgMapping, FileContent, HgChangeset, HgFileEnvelope, HgFileNode, HgManifest] (glob)
  * Final count: (17, 17) (glob)
  * Type:Walked,Checks,Children BonsaiHgMapping:1,1,1 Bookmark:1,1,1 FileContent:3,3,0 HgChangeset:3,3,5 HgFileEnvelope:3,*,3 HgFileNode:3,6,0 HgManifest:3,3,6  (glob)
  * Exiting... (glob)
...
count-objects, default shallow walk across bonsai and hg data
  ...
  * Walking roots * (glob)
  * Walking edge types [BonsaiChangesetToBonsaiHgMapping, BonsaiChangesetToFileContent, BonsaiHgMappingToHgChangeset, BookmarkToBonsaiChangeset, FileContentMetadataToGitSha1Alias, FileContentMetadataToSha1Alias, FileContentMetadataToSha256Alias, FileContentToFileContentMetadata, HgBonsaiMappingToBonsaiChangeset, HgChangesetToHgManifest, HgFileEnvelopeToFileContent, HgManifestToChildHgManifest, HgManifestToHgFileEnvelope, HgManifestToHgFileNode] (glob)
  * Walking node types [BonsaiChangeset, BonsaiHgMapping, FileContent, FileContentMetadata, HgChangeset, HgFileEnvelope, HgFileNode, HgManifest] (glob)
  * Final count: (17, 17) (glob)
