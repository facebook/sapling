...
class TestPushDirectories(test_hgsubversion_util.TestBase):
    ...
    def test_push_new_dir_project_root_not_repo_root(self):
        ...
        self.pushrevisions()
        self.assertEqual(
            test_hgsubversion_util.svnls(repo_path, "foo/trunk"),
            [
                "bar",
                "bar/alpha",
                "bar/beta",
    ...
    def test_push_new_file_existing_dir_root_not_repo_root(self):
        repo_path = self.load_and_fetch(
            "empty_dir_in_trunk_not_repo_root.svndump", subdir="project"
        )[1]
        changes = [("narf/a", "narf/a", "ohai")]
        self.commitchanges(changes)
        self.assertEqual(
            test_hgsubversion_util.svnls(repo_path, "project/trunk"), ["a", "narf"]
        )
        self.pushrevisions()
        self.assertEqual(
            test_hgsubversion_util.svnls(repo_path, "project/trunk"),
            ["a", "narf", "narf/a"],
        )
        ...
        self.pushrevisions()
        self.assertEqual(
            test_hgsubversion_util.svnls(repo_path, "project/trunk"), ["a"]
        )
    ...
    def test_push_single_dir_change_in_subdir(self):
        ...
        repo, repo_path = self.load_and_fetch(
            "branch_from_tag.svndump", layout="single", subdir="tags"
        )
        changes = [
            ("tag_r3/alpha", "tag_r3/alpha", "foo"),
        ...
        self.pushrevisions()
        self.assertEqual(
            test_hgsubversion_util.svnls(repo_path, "tags"),
            [
                "copied_tag",
                "copied_tag/alpha",
                "copied_tag/beta",
