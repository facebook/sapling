...
class TestFetchBranches(test_hgsubversion_util.TestBase):
    ...
    def branches(self, repo):
        ...
        openbranches = set(
            ctx.branch() for ctx in hctxs if ctx.extra().get("close", None) != "1"
        )
        closedbranches = set(
            ctx.branch() for ctx in hctxs if ctx.extra().get("close", None) == "1"
        )
    ...
    def test_unrelatedbranch(self):
        ...
        self.assertEqual(sorted(heads["branch2"].manifest().keys()), ["a", "b"])
    ...
    def test_branch_create_with_dir_delete_works(self):
        ...
        self.assertEqual(
            sorted(repo["tip"].manifest().keys()), ["alpha", "beta", "gamma", "iota"]
        )

    def test_branch_tip_update_to_default(self):
        repo = self._load_fixture_and_fetch("unorderedbranch.svndump", noupdate=False)
    ...
    def test_branch_pull_anchor(self):
        ...
        repo = self._load_fixture_and_fetch_with_anchor("unorderedbranch.svndump", "4")
        self.assertTrue("c" not in compathacks.branchset(repo))

    def test_branches_weird_moves(self):
        repo = self._load_fixture_and_fetch("renamedproject.svndump", subdir="project")
        heads = [repo[n] for n in repo.heads()]
        heads = dict((ctx.branch(), ctx) for ctx in heads)
    ...
    def test_misspelled_branches_tags(self):
        config = {"hgsubversion.branchdir": "branchez", "hgsubversion.tagpaths": "tagz"}
        """Tests using the tags dir for branches and the branches dir for tags"""
        repo = self._load_fixture_and_fetch(
            "misspelled_branches_tags.svndump", layout="standard", config=config
        )
    ...
    def test_subdir_branches_tags(self):
        ...
        repo = self._load_fixture_and_fetch(
            "subdir_branches_tags.svndump", layout="standard", config=config
        )
    ...
    def test_subproject_fetch(self):
        config = {"hgsubversion.infix": "project"}
        repo = self._load_fixture_and_fetch(
            "subprojects.svndump", layout="standard", config=config
        )
