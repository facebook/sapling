...
class MapTests(test_hgsubversion_util.TestBase):
    ...
    def test_author_map(self):
        ...
        commands.clone(
            ui,
            test_hgsubversion_util.fileurl(repo_path),
            self.wc_path,
            authors=self.authors,
        )
        self.assertEqual(self.repo[0].user(), "Augie Fackler <durin42@gmail.com>")
        self.assertEqual(
            self.repo["tip"].user(), "evil@5b65bade-98f3-4993-a01f-b7a6710da339"
        )
    ...
    def test_author_map_closing_author(self):
        ...
        self.assertEqual(
            self.repo[0].user(), "Augie@5b65bade-98f3-4993-a01f-b7a6710da339"
        )
        self.assertEqual(self.repo["tip"].user(), "Testy <test@test>")
    ...
    def _loadwithfilemap(self, svndump, filemapcontent, failonmissing=True):
    ...
    def test_file_map_rule_order(self):
        ...
        self.assertEqual(sorted(self.repo[0].manifest().keys()), ["alpha", "beta"])
        self.assertEqual(
            sorted(self.repo["default"].manifest().keys()), ["alpha", "beta"]
        )
    ...
    def test_branchmap_combine(self):
        ...
        self.assertEquals(sorted(branches.keys()), [None, "badname", "feature"])
