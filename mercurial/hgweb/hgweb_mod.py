...
class requestcontext(object):
    ...
    def configbool(self, section, name, default=uimod._unset, untrusted=True):
        return self.repo.ui.configbool(section, name, default, untrusted=untrusted)

    def configint(self, section, name, default=uimod._unset, untrusted=True):
        return self.repo.ui.configint(section, name, default, untrusted=untrusted)

    def configlist(self, section, name, default=uimod._unset, untrusted=True):
        return self.repo.ui.configlist(section, name, default, untrusted=untrusted)
    ...
    def templater(self, req):
        ...
        tmpl = templater.templater.frommapfile(
            mapfile, filters={"websub": websubfilter}, defaults=defaults
        )
        return tmpl
...
class hgweb(object):
    ...
    def run(self):
        ...
        method, if possible.
        """
        if not encoding.environ.get("GATEWAY_INTERFACE", "").startswith("CGI/1."):
            raise RuntimeError(
