...
pub trait Bookmarks: Send + Sync + 'static {
    ...
    fn list_by_prefix(
        ...
        prefix: &AsciiString,
        repoid: &RepositoryId,
    ) -> BoxStream<(AsciiString, ChangesetId), Error>;
...
pub trait Transaction: Send + Sync + 'static {
    ...
    fn create(&mut self, key: &AsciiString, new_cs: &ChangesetId) -> Result<()>;
    ...
    /// exists or not.
    fn force_set(&mut self, key: &AsciiString, new_cs: &ChangesetId) -> Result<()>;

    /// Adds delete operation to the transaction set.
    /// Deletes bookmark only if it currently points to `old_cs`.
    fn delete(&mut self, key: &AsciiString, old_cs: &ChangesetId) -> Result<()>;
