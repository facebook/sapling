...
class GitHandler(object):
    ...
    def push(self, remote, revs, force):
        ...
        if remote_name and new_refs:
            for ref, new_sha in sorted(new_refs.iteritems()):
                ...
                if old_sha is None:
                    if self.ui.verbose:
                        self.ui.note(_("adding reference %s::%s => GIT:%s\n") %
    ...
    def get_git_message_and_extra(self, ctx):
        ...
        for i, field, value in git_extraitems:
            git_extra.append((hgutil.urlreq.unquote(field),
                              hgutil.urlreq.unquote(value)))
            ...
            if renames:
                for oldfile, newfile in renames:
                    if extra_in_message:
                        ...
                        spec = '%s:%s' % (hgutil.urlreq.quote(oldfile),
        ...
        for key, value in extraitems:
            if key in ('author', 'committer', 'encoding', 'message', 'branch',
                ...
                if extra_in_message:
                    ...
                    spec = '%s:%s' % (hgutil.urlreq.quote(key),
                                      hgutil.urlreq.quote(value))
    ...
    def import_git_commit(self, commit):
        ...
        def getfilectx(repo, memctx, f):
            ...
            try:
                return context.memfilectx(self.repo, memctx, f, data,
                                          islink='l' in e,
                                          isexec='x' in e,
                                          copied=copied_path)
            except TypeError:
            ...
            if self.repo.tagtype(tag) in ('global', 'git'):
                ...
                if target is not None:
                    ...
                    if check_ref_format(tag_refname):
                        ...
                        self.repo.ui.warn(_('Skipping export of tag %s because '
                                          'it has invalid name as a git '
                                          'refname.\n') % tag)
                else:
                    self.repo.ui.warn(
                        _('Skipping export of tag %s because it '
