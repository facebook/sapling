...
class _httprequesthandler(httpservermod.basehttprequesthandler):
    ...
    def log_request(self, code=r"-", size=r"-"):
        ...
        if util.safehasattr(self, "headers"):
            xheaders = [h for h in self.headers.items() if h[0].startswith(r"x-")]
        self.log_message(
            r'"%s" %s %s%s',
            self.requestline,
            str(code),
    ...
    def do_POST(self):
        try:
            ...
            newline = chr(10)
            self.log_error(
                r"Exception happened during processing " r"request '%s':%s%s",
                self.path,
                newline,
                tb,
            )
    ...
    def do_hgweb(self):
        ...
        env[r"wsgi.multithread"] = isinstance(self.server, socketserver.ThreadingMixIn)
        env[r"wsgi.multiprocess"] = isinstance(self.server, socketserver.ForkingMixIn)
    ...
    def _start_response(self, http_status, headers, exc_info=None):
        ...
        self.saved_headers = [h for h in headers if h[0].lower() not in bad_headers]
        return self._write
