...
def urlrepos(prefix, roothead, paths):
    ...
    for path in paths:
        ...
        yield (prefix + "/" + util.pconvert(path[len(roothead) :]).lstrip("/")).strip(
            "/"
        ), path
...
class hgwebdir(object):
    ...
    def refresh(self):
        ...
        if refreshinterval > 0 and self.lastrefresh + refreshinterval > time.time():
            return
    ...
    def _runwsgi(self, req):
        try:
            ...
            if virtual.startswith("static/") or "static" in req.form:
                ...
                static = self.ui.config("web", "static", None, untrusted=False)
                if not static:
    ...
    def makeindex(self, req, tmpl, subdir=""):
        ...
        sort = [
            (
                "sort_%s" % column,
                "%s%s"
                % ((not descending and column == sortcolumn) and "-" or "", column),
            )
    ...
    def templater(self, req, nonce):
        ...
        vars = {}
        styles, (style, mapfile) = hgweb_mod.getstyle(req, config, self.templatepath)
        if style == styles[0]:
            vars["style"] = style
