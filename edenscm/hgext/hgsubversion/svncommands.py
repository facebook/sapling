...
def _buildmeta(ui, repo, args, partial=False, skipuuid=False):
    ...
    if meta.subdir is None:
        subdir = (
            ui.config("hgsubversion", "reposubdir")
            or svnrepo.svnremoterepo(ui, url).svn.subdir
        )
        meta.subdir = subdir
    ...
    with progress.bar(ui, "rebuild", total=numrevs) as prog:
        for ctx in util.get_contexts(repo, startrev):
            ...
            if meta.uuid is None or validateuuid:
                ...
                uuid = convinfo[4:40]
                if not skipuuid:
                    uuidfromconforserver = (
                        ui.config("hgsubversion", "repouuid")
                        or svnrepo.svnremoterepo(ui, url).svn.uuid
                    )
