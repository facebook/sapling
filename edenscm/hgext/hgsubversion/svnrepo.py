...
def generate_repo_class(ui, repo):
    ...
    def remotesvn(fn):
        ...
        def wrapper(self, *args, **opts):
            original = getattr(self, fn.__name__, None)
    ...
    class svnlocalrepo(superclass):
        def svn_commitctx(self, ctx):
            ...
            with self.ui.configoverride({("phases", "new-commit"): "public"}, "hgsvn"):
...
class svnremoterepo(peerrepository):
    ...
    def __init__(self, ui, path=None):
        ...
        if not path:
            raise hgutil.Abort(
                "no Subversion URL specified. Expect "
    ...
    def svn(self):
        try:
            auth = self.svnauth
            return svnwrap.SubversionRepo(
                auth[0], auth[1], auth[2], password_stores=self.password_stores
            )
