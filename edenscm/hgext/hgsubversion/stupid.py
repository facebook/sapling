...
headers_re = re.compile("(?:" + "|".join([index_header, property_header]) + ")")
...
def print_your_svn_is_old_message(ui):  # pragma: no cover
    ui.status(
        "In light of that, I'll fall back and do diffs, but it won't do "
...
def diff_branchrev(ui, svn, meta, branch, branchpath, r, parentctx):
    ...
    try:
        if prev is None or pbranch == branch:
            ...
            d = svn.get_unified_diff(
                branchpath, r.revnum, other_rev=prev, deleted=False, ignore_type=False
            )
        else:
            d = svn.get_unified_diff(
                branchpath,
    ...
    copies = getcopies(svn, meta, branch, branchpath, r, touched_files, parentctx)
    ...
    exec_files = dict(
        (f.name, f.executable) for f in changed if f.executable is not None
    )
    link_files = dict((f.name, f.symlink) for f in changed if f.symlink is not None)
...
def branches_in_paths(meta, tbdelta, paths, revnum, checkpath, listdir, firstrun):
    """Given a list of paths, return mapping of all branches touched
    to their branch path.
    """
    branches = {}
    if firstrun:
        paths_need_discovery = [p for (p, t) in listdir("", revnum) if t == "f"]
    else:
    ...
    while paths_need_discovery:
        ...
        # seems to be the best option.
            ...
            paths_need_discovery.extend(
                ["%s/%s" % (p, x[0]) for x in listdir(p, revnum) if x[1] == "f"]
            )
...
def convert_rev(ui, meta, svn, r, tbdelta, firstrun):
    ...
    branches = branches_in_paths(
        meta, tbdelta, r.paths, r.revnum, svn.checkpath, svn.list_files, firstrun
    )
    ...
    for p in r.paths:
        ...
        frompath, frombranch = meta.split_branch_path(frompath, existing=False)[:2]
        if frompath == "":
            ...
            if fromnode != node.nullid:
                ...
                pctx = meta.repo[meta.get_parent_revision(r.revnum, branch, exact=True)]
    ...
    for b in branches:
        ...
        # the latter for now.
        incremental = (
            meta.revmap.firstpulled > 0
            and parentctx.rev() != node.nullrev
            and not firstrun
        )
        ...
        if meta.layout != "single":
            ...
            externals = svnexternals.getchanges(ui, meta.repo, parentctx, externals)
        ...
        if parentctx.node() == node.nullid and not files_touched:
            meta.repo.ui.debug(
                "skipping commit since parent is null and no files touched.\n"
            )
            continue
        ...
        if not tag:
            if not origbranch in meta.branches and not meta.get_path_tag(
                meta.remotename(origbranch)
            ):
