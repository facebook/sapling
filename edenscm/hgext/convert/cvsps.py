...
from edenscm.mercurial import encoding, error, hook, pycompat, util
...
def createlog(ui, directory=None, root="", rlog=True, cache=None):
    ...
    re_03 = re.compile(
        "(Cannot access.+CVSROOT)|" "(can't create temporary directory.+)$"
    )
    ...
    while True:
        ...
        if state == 0:
            ...
            if len(d.split()) != 3:
                ...
                d = d + " UTC"
            e.date = util.parsedate(
                d, ["%y/%m/%d %H:%M:%S", "%Y/%m/%d %H:%M:%S", "%Y-%m-%d %H:%M:%S"]
            )
        ...
        for entry in log:
            ...
            for e in encodings:
                try:
                    ...
                    raise error.Abort(
                        inst, hint=_("check convert.cvsps.logencoding" " configuration")
                    )
            else:
                raise error.Abort(
                    _("no encoding can transcode" " CVS log message for %s of %s")
        ...
        if not off:
            ...
            for f in cs.entries:
                ...
                ui.write(
                    ...
                    % (
                        fn,
                        ".".join([str(x) for x in f.parent]) or "INITIAL",
