...
from mercurial import cmdutil, error, mail, registrar, url, util
...
configitem("bugzilla", "apikey", default="")
configitem("bugzilla", "bzdir", default="/var/www/html/bugzilla")
configitem("bugzilla", "bzemail", default=None)
configitem("bugzilla", "bzurl", default="http://localhost/bugzilla/")
configitem("bugzilla", "bzuser", default=None)
configitem("bugzilla", "db", default="bugs")
configitem(
    "bugzilla",
    "fixregexp",
    default=(
...
configitem("bugzilla", "fixresolution", default="FIXED")
configitem("bugzilla", "fixstatus", default="RESOLVED")
configitem("bugzilla", "host", default="localhost")
configitem("bugzilla", "notify", default=configitem.dynamicdefault)
configitem("bugzilla", "password", default=None)
configitem(
    "bugzilla",
    "regexp",
    default=(
        r"bugs?\s*,?\s*(?:#|nos?\.?|num(?:ber)?s?)?\s*"
        r"(?P<ids>(?:\d+\s*(?:,?\s*(?:and)?)?\s*)+)"
        r"\.?\s*(?:h(?:ours?)?\s*(?P<hours>\d*(?:\.\d+)?))?"
    ),
)
configitem("bugzilla", "strip", default=0)
configitem("bugzilla", "style", default=None)
configitem("bugzilla", "template", default=None)
configitem("bugzilla", "timeout", default=5)
configitem("bugzilla", "user", default="bugs")
configitem("bugzilla", "usermap", default=None)
configitem("bugzilla", "version", default=None)
    ...
    def get_bugzilla_user(self, committer):
        ...
        try:
            ...
            try:
                ...
                if not defaultuser:
                    raise error.Abort(_("cannot find bugzilla user id for %s") % user)
                userid = self.get_user_id(defaultuser)
                user = defaultuser
            except KeyError:
                raise error.Abort(
                    _("cannot find bugzilla user id for %s or %s") % (user, defaultuser)
                )
        return (user, userid)
...
class bzxmlrpcemail(bzxmlrpc):
    ...
    def send_bug_modify_email(self, bugid, commands, comment, committer):
        ...
        """
        user = self.map_committer(committer)
        matches = self.bzproxy.User.get({"match": [user], "token": self.bztoken})
        if not matches["users"]:
            user = self.ui.config("bugzilla", "user")
            matches = self.bzproxy.User.get({"match": [user], "token": self.bztoken})
            if not matches["users"]:
                raise error.Abort(_("default bugzilla user %s email not found") % user)
...
class bzrestapi(bzaccess):
    ...
    def updatebug(self, bugid, newstate, text, committer):
        ...
        if bugmod:
            ...
            self._submit(
                burl, {"comment": text, "is_private": False, "is_markdown": False}
            )
...
class bugzilla(object):
    ...
    def __init__(self, ui, repo):
        ...
        try:
            ...
            raise error.Abort(_("bugzilla version %s not supported") % bzversion)
        self.bzdriver = bzclass(self.ui)

        self.bug_re = re.compile(self.ui.config("bugzilla", "regexp"), re.IGNORECASE)
        self.fix_re = re.compile(self.ui.config("bugzilla", "fixregexp"), re.IGNORECASE)
    ...
    def update(self, bugid, newstate, ctx):
        ...
        t = cmdutil.changeset_templater(self.ui, self.repo, spec, False, None, False)
        self.ui.pushbuffer()
        t.show(
            ctx,
