...
static inline size_t read_noint(FILE *fh, void *_buf, size_t nbytes) {
  ...
  while (read_so_far < nbytes) {
    size_t read_this_iteration = fread(
        buf + read_so_far,
        1,
        nbytes - read_so_far,
        fh);
...
read_from_file_result_t read_from_file(char *fname, size_t fname_sz) {
  ...
  node_t *real_root = (node_t *) ( ((intptr_t) result.tree->arena) +
                                   header.root_offset );
...
static inline size_t write_noint(FILE *fh, void *_buf, size_t nbytes) {
  ...
  while (written_so_far < nbytes) {
    size_t written_this_iteration = fwrite(
        buf + written_so_far,
        1,
        nbytes - written_so_far,
        fh);
...
static write_to_file_result_t write_compact_tree_to_file(
...
static void initialize_unused_bytes(node_t *node) {
  ...
  memset(&node->checksum[node->checksum_sz], 0,
...
write_to_file_result_t write_to_file_helper(
  ...
  write_to_file_result_t result = write_compact_tree_to_file(
      compact_copy, fname, fname_sz);
...
write_to_file_result_t write_to_file(
    tree_t *tree, char *fname, size_t fname_sz) {
