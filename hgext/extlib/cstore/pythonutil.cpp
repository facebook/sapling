...
{
  PythonObj arglist =
      Py_BuildValue("s#s#", key.name.c_str(), (Py_ssize_t)key.name.size(),
  ...
  if (!result) {
    ...
    PyErr_Format(PyExc_RuntimeError, "unable to find tree '%.*s:...'",
                 (int)key.name.size(), key.name.c_str());
    throw pyexception();
  }
...
void PythonDiffResult::add(const std::string &path, const char *beforeNode,
                           ...
                           const char *afterFlag)
{
  ...
  PythonObj entry =
      Py_BuildValue("((s#s#)(s#s#))", beforeNode, beforeLen,
                    (beforeFlag == NULL) ? MAGIC_EMPTY_STRING : beforeFlag,
                    Py_ssize_t(beforeFlag ? 1 : 0), afterNode, afterLen,
