...
static PyObject *datapackstore_getdeltachain(py_datapackstore *self,
                                             PyObject *args)
{
  try {
    char *name;
    ...
    for (DeltaChainLink link = chain.next(); !link.isdone();
      ...
      PythonObj delta = PyString_FromStringAndSize((const char *)link.delta(),
                                                   (Py_ssize_t)link.deltasz());

      PythonObj tuple = PyTuple_Pack(
          5, (PyObject *)name, (PyObject *)retnode, (PyObject *)name,
          (PyObject *)deltabasenode, (PyObject *)delta);
...
static PyObject *datapackstore_getmissing(py_datapackstore *self,
                                          PyObject *keys)
{
  try {
    ...
    while ((key = missingIter->next()) != NULL) {
      PythonObj missingKey = Py_BuildValue("(s#s#)", key->name.c_str(),
...
static PyObject *uniondatapackstore_getdeltachain(py_uniondatapackstore *self,
                                                  PyObject *args)
{
  try {
    char *name;
...
static PyObject *uniondatapackstore_getmissing(py_uniondatapackstore *self,
                                               PyObject *keys)
{
  try {
    PythonObj result = PyList_New(0);
...
static PyMethodDef uniondatapackstore_methods[] = {
    ...
    {"markforrefresh", (PyCFunction)uniondatapackstore_markforrefresh,
     METH_NOARGS, ""},
