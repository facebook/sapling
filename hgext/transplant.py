...
configitem("transplant", "filter", default=None)
configitem("transplant", "log", default=None)
...
class transplanter(object):
    ...
    def applied(self, repo, node, parent):
        ...
        if hasnode(repo, node):
            ...
            reachable = repo.changelog.ancestors([parentrev], rev, inclusive=True)
            if rev in reachable:
                return True
    ...
    def filter(self, filter, node, changelog, patchfile):
        ...
        try:
            self.ui.system(
                "%s %s %s"
                % (filter, util.shellquote(headerfile), util.shellquote(patchfile)),
                environ={"HGUSER": changelog[1], "HGREVISION": nodemod.hex(node)},
                onerr=error.Abort,
    ...
    def applyone(self, repo, node, cl, patchfile, merge=False, log=False, filter=None):
        ...
        n = repo.commit(
            message, user, date, extra=extra, match=m, editor=self.getcommiteditor()
        )
        if not n:
    ...
    def recover(self, repo, source, opts):
        ...
        try:
            ...
            if merge or modified or added or removed or deleted:
                n = repo.commit(
                    message, user, date, extra=extra, editor=self.getcommiteditor()
                )
                if not n:
...
@command(
    ...
    [
        ...
        ("", "parent", "", _("parent to choose when transplanting merge"), _("REV")),
...
def _dotransplant(ui, repo, *revs, **opts):
    ...
    def checkopts(opts, revs):
        if opts.get("continue"):
            if opts.get("branch") or opts.get("all") or opts.get("merge"):
                raise error.Abort(
                    _("--continue is incompatible with " "--branch, --all and --merge")
                )
            return
        if not (opts.get("source") or revs or opts.get("merge") or opts.get("branch")):
            raise error.Abort(
                _("no source URL, branch revision, or revision " "list provided")
            )
        if opts.get("all"):
            if not opts.get("branch"):
                raise error.Abort(_("--all requires a branch revision"))
            if revs:
                raise error.Abort(_("--all is incompatible with a " "revision list"))
    ...
    try:
        ...
        if opts.get("prune"):
            prune = set(
                source.lookup(r) for r in scmutil.revrange(source, opts.get("prune"))
            )
        ...
        if revs:
            ...
            if source != repo:
                ...
                alltransplants = transplantwalk(source, p1, heads, match=matchfn)
            if opts.get("all"):
                revs = alltransplants
            else:
...
def revsettransplanted(repo, subset, x):
    ...
    if x:
        ...
        s = subset
    return smartset.baseset([r for r in s if repo[r].extra().get("transplant_source")])
