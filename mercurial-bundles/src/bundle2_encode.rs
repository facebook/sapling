    ...
    W: AsyncWrite + Send,
{
    pub fn new(writer: W) -> Self {
        Bundle2EncodeBuilder {
            ...
            compressor_type: CompressorType::Uncompressed,
            parts: Vec::new(),
        }
    }
    ...
    pub fn set_compressor_type(&mut self, ct: CompressorType) -> &mut Self {
        self.compressor_type = ct;
        self
    }
...
type PartSink<W> = FramedWrite<Compressor<W>, ChunkEncoder>;
    ...
    W: AsyncWrite + Send,
{
    fn finish(self) -> (IntoIter<PartEncode>, PartSink<W>) {
        ...
        (
            ...
            FramedWrite::new(
                Compressor::new(self.writer, self.compressor_type),
                ChunkEncoder,
            ),
        )
    }
    ...
    W: AsyncWrite + Send,
{
    fn poll_next(state: EncodeState<W>) -> (Poll<W, Error>, EncodeState<W>) {
        match state {
            ...
            EncodeState::Finish(compressor) => Self::poll_finish(compressor),
    ...
    fn poll_eos(mut sink: PartSink<W>, eos_written: bool) -> (Poll<W, Error>, EncodeState<W>) {
        ...
        match sink.poll_complete() {
            Ok(Async::Ready(())) => Self::poll_finish(sink.into_inner()),
