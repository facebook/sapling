...
py_class!(pub class mergestate |py| {
    ...
    def add_subtree_merge(
        &self,
        from_commit: PyBytes,
        from_path: PyPathBuf,
        to_path: PyPathBuf,
        from_repo_url: Option<String>
    ) -> PyResult<PyNone> {
        ...
        self.ms(py).borrow_mut().add_subtree_merge(from_commit, from_path, to_path, from_repo_url);
        Ok(PyNone)
    }

    def subtree_merges(&self) -> PyResult<Serde<Vec<SubtreeMerge>>> {
        let data = self
            .ms(py)
            .borrow()
            .subtree_merges()
