...
def preimport(name):
    ...
    mod = getattr(__import__("sapling.ext.%s" % name).ext, name)
...
def loaddefault(name, reportfunc=None):
    ...
    if mod:
        return mod
    mod = _importh("sapling.ext.%s" % name)
    return mod
...
def _importext(name, path=None, reportfunc=None):
    if path:
        ...
        mod = loadpath(path, "sapling.ext.%s" % name)
    else:
...
def disabled():
    ...
    try:
        # pyre-fixme[21]: Could not find name `__index__` in `sapling.ext`.
        from sapling.ext import __index__
