    ...
    if workflow == SubmitWorkflow.STACKED:
        ...
        if not repository.is_fork:
            ...
            if local_stack:
                ...
                github_stack_fetched = fetched
                if github_stack:
                    stack_prs = github_stack.pull_requests
                    if stack_prs == local_stack[: len(stack_prs)]:
                        ...
                        ui.status_err(
                            _(
                                "stack #%d on GitHub (%s) does not match your "
                                "local stack (%s); not updating it\n"
                            )
                            % (
                                github_stack.number,
...
async def rewrite_pull_request_body(
    ...
    if workflow == SubmitWorkflow.STACKED:
        ...
        base = None
    elif workflow.uses_chained_bases() and index < len(partitions) - 1:
        base = none_throws(partitions[index + 1][0].head_branch_name)
