...
use crate::{
    ...
    scmstore::{
        ...
        FallbackCache, FilterMapStore, LegacyDatastore, StoreFile, StoreTree,
    },
    util::{get_cache_path, get_indexedlogdatastore_path, get_repo_name},
    ContentStore, EdenApiFileStore, EdenApiTreeStore, ExtStoredPolicy,
};
...
pub struct TreeScmStoreBuilder<'a> {
    config: &'a ConfigSet,
    suffix: Option<PathBuf>,
    shared_indexedlog: Option<Arc<IndexedLogHgIdDataStore>>,
    shared_edenapi: Option<Arc<EdenApiTreeStore>>,
    legacy_datastore: Option<LegacyDatastore<Arc<ContentStore>>>,
}

impl<'a> TreeScmStoreBuilder<'a> {
    pub fn new(config: &'a ConfigSet) -> Self {
        Self {
            config,
            suffix: None,
    ...
    pub fn shared_edenapi(mut self, shared_edenapi: Arc<EdenApiTreeStore>) -> Self {
        self.shared_edenapi = Some(shared_edenapi);
        self
    }
    ...
    fn build_edenapi(&self) -> Result<Arc<EdenApiAdapter<Client>>> {
        ...
        Ok(Arc::new(EdenApiAdapter {
            client: Builder::from_config(self.config)?.build()?,
            repo: reponame,
            // TODO(meyer): IF we expose a `build_shared_indexedlog`-like method to get a copy of the EdenApi client built by
            // the TreeScmStoreBuilder, we'll actually need to respect the extstored_policy, or else it could accidentally be
            // used for files with the wrong configuration. Alternatively, we can split `EdenApiAdapter` into a `files` and
            // `trees` variant, and only include extstored_policy on the `files` one. I took this approach in an earlier draft,
            // but abandoned it for now since we're intending to remove ExtStoredPolicy eventually and it leads to some
    ...
    pub fn build(self) -> Result<BoxedReadStore<Key, StoreTree>> {
        ...
        let edenapi = if self.use_edenapi()? {
            if let Some(shared_edenapi) = self.shared_edenapi.as_ref() {
                // TOD(meyer): Same unnecessary ExtStoredPolicy issue here as above
                Some(
                    Arc::new(shared_edenapi.get_scmstore_adapter(ExtStoredPolicy::Use))
                        as BoxedReadStore<Key, TreeEntry>,
                )
            } else {
                Some(self.build_edenapi()? as BoxedReadStore<Key, TreeEntry>)
            }
        } else {
            None
        };
        ...
        Ok(match (edenapi, self.legacy_datastore) {
            (Some(edenapi), Some(legacy)) => {
                ...
                Arc::new(FallbackCache {
                    preferred: indexedlog.clone(),
                    fallback: legacy_fallback as BoxedReadStore<Key, Entry>,
                    write_store: Some(indexedlog),
                })
            }
            (None, Some(legacy)) => Arc::new(FallbackCache {
                preferred: indexedlog.clone(),
            ...
            // TODO(meyer): Strongly type this.
            _ => bail!("Unsupported TreeScmStoreBuilder Configuration"),
        })
    }
}
