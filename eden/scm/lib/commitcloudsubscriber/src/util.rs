...
#[derive(Clone, PartialEq, Debug)]
pub enum CatTokenVerifier {
    Icebreaker,
    InternGraph,
}

impl fmt::Display for CatTokenVerifier {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            CatTokenVerifier::Icebreaker => write!(f, "commitcloud-service"),
            CatTokenVerifier::InternGraph => write!(f, "interngraph"),
        }
    }
}
...
pub fn read_or_generate_access_token(
    user_token_path: &Option<PathBuf>,
    cat_token_verifier: CatTokenVerifier,
) -> Result<Token> {
    ...
    info!(
        "Token Lookup: generating a CAT token via {} for verifier '{}' ...",
        clicat_tool, cat_token_verifier
    );
    let token_timeout_seconds = 1200;
    let output = match cat_token_verifier {
        CatTokenVerifier::Icebreaker => Command::new(clicat_tool)
            .args(vec![
                "create",
                "--verifier_type",
                "SERVICE_IDENTITY",
