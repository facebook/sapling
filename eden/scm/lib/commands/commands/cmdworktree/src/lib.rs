...
fn run_label(ctx: &ReqCtx<WorktreeOpts>, repo: &Repo) -> Result<u8> {
    let logger = ctx.logger();
    let (shared_store_path, group_id) = require_group(repo)?;
...
/// Parse positional args for `sl worktree label [PATH] TEXT [--remove]`.
///
/// The label text can come from either a positional argument or the `--label`
/// flag.  It is an error to supply both.
///
/// With `--remove`: target defaults to current worktree, or PATH if given.
/// Without `--remove`: 1 arg = label for current worktree, 2 args = PATH + label.
fn parse_label_args(ctx: &ReqCtx<WorktreeOpts>, repo: &Repo) -> Result<(PathBuf, String)> {
    ...
    if ctx.opts.remove {
        ...
        return match args.len() {
            ...
            _ => {
                abort!("usage: sl worktree label [PATH] --remove");
            }
        };
    }
    ...
    match args.len() {
        ...
        2 => {
            if let Some(label) = flag_label {
                // "sl worktree label PATH --label TEXT"
                let target = util::path::strip_unc_prefix(fs::canonicalize(&args[1])?);
                Ok((target, label))
            } else {
                // "sl worktree label TEXT" => label for current worktree
