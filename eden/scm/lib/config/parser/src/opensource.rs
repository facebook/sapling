...
sl_node="{label(sl_phase_label, ifeq(phase, 'draft', hyperlink(separate('', 'https://www.internalfb.com/intern/commit/cloud/FBS/', node), shortest(node, sl_hash_minlen)), shortest(node, sl_hash_minlen)))}"
