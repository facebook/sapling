...
enum dir_state { DDOT, DH, DHGDI, DDEFAULT };
...
static inline void
charcopy(char* dest, Py_ssize_t* destlen, size_t destsize, char c) {
  if (dest) {
...
static inline void
hexencode(char* dest, Py_ssize_t* destlen, size_t destsize, uint8_t c) {
...
static inline void
escape3(char* dest, Py_ssize_t* destlen, size_t destsize, char c) {
  charcopy(dest, destlen, destsize, '~');
  hexencode(dest, destlen, destsize, c);
}

static Py_ssize_t
_encodedir(char* dest, size_t destsize, const char* src, Py_ssize_t len) {
...
static Py_ssize_t
basicencode(char* dest, size_t destsize, const char* src, Py_ssize_t len) {
...
static Py_ssize_t
_lowerencode(char* dest, size_t destsize, const char* src, Py_ssize_t len) {
  static const uint32_t onebyte[8] = {1, 0x2bfffbfb, 0xe8000001, 0x2fffffff};
...
static Py_ssize_t
auxencode(char* dest, size_t destsize, const char* src, Py_ssize_t len) {
...
static PyObject*
hashmangle(const char* src, Py_ssize_t len, const char sha[20]) {
  ...
  if (slop > 0) {
    Py_ssize_t basenamelen = lastslash >= 0 ? len - lastslash - 2 : len - 1;

    if (basenamelen > slop)
      basenamelen = slop;
    if (basenamelen > 0)
      memcopy(dest, &destlen, destsize, &src[lastslash + 1], basenamelen);
  }
  ...
  if (lastdot >= 0)
    memcopy(dest, &destlen, destsize, &src[lastdot], len - lastdot - 1);
...
static int sha1hash(char hash[20], const char* str, Py_ssize_t len) {
  ...
  if (!PyBytes_Check(hashobj) || PyBytes_GET_SIZE(hashobj) != 20) {
    PyErr_SetString(PyExc_TypeError, "result of digest is not a 20-byte hash");
    Py_DECREF(hashobj);
    return -1;
  }
