    ...
    * serverhostname - The expected hostname of the remote server. If the
    ...
    if settings["ciphers"]:
        try:
            ...
            raise error.Abort(
                _("could not set ciphers: %s") % e.args[0],
                hint=_("change cipher string (%s) in config") % settings["ciphers"],
            )
    ...
    if settings["cafile"] is not None:
        try:
            ...
            if len(e.args) == 1:  # pypy has different SSLError args
                ...
                msg = e.args[1]
            raise error.Abort(
                _("error loading CA file %s: %s") % (settings["cafile"], msg),
                ...
                if settings["protocolui"] == "tls1.0":
                    ...
                    if supportedprotocols != {"tls1.0"}:
                        ui.warn(
                            ...
                            % (serverhostname, ", ".join(sorted(supportedprotocols)))
                        )
                    else:
                        ui.warn(
                            _(
...
def _dnsnamematch(dn, hostname, maxwildcards=1):
    ...
    if wildcards > maxwildcards:
        raise wildcarderror(_("too many wildcards in certificate DNS name: %s") % dn)
