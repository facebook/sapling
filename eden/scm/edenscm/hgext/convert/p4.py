...
class p4_source(common.converter_source):
    def __init__(self, ui, repotype, path, revs=None):
        ...
        if "/" in path and not path.startswith("//"):
            raise common.NoRepo(_("%s does not look like a P4 repository") % path)

        common.checktool("p4", abort=False)

        self.revmap = {}
        self.encoding = self.ui.config("convert", "p4.encoding", convcmd.orig_encoding)
        self.re_type = re.compile(
            "([a-z]+)?(text|binary|symlink|apple|resource|unicode|utf\d+)" "(\+\w+)?$"
        )
        self.re_keywords = re.compile(
            r"\$(Id|Header|Date|DateTime|Change|File|Revision|Author)" r":[^$\n]*\$"
        )
