...
def getrepophid(repo):
    ...
    if not callsign:
        return None
    query = callconduit(
        repo, "diffusion.repository.search", {"constraints": {"callsigns": [callsign]}}
    )
...
_differentialrevisiontagre = re.compile(r"\AD([1-9][0-9]*)\Z")
_differentialrevisiondescre = re.compile(
    r"^Differential Revision:\s*(?:.*)D([1-9][0-9]*)$", re.M
)
...
def getoldnodedrevmap(repo, nodelist):
    ...
    if toconfirm:
        ...
        for newnode, (force, precset, drev) in toconfirm.items():
            diffs = [d for d in alldiffs.values() if int(d[r"revisionID"]) == drev]
...
def createdifferentialrevision(
    ctx, revid=None, parentrevid=None, oldnode=None, olddiff=None, actions=None
):
    ...
    info = callconduit(repo, "differential.parsecommitmessage", {"corpus": desc})
...
def userphids(repo, names):
    ...
    if unresolved:
        raise error.Abort(_("unknown username: %s") % " ".join(sorted(unresolved)))
...
def phabsend(ui, repo, *revs, **opts):
    ...
    for rev in revs:
        ...
        actiondesc = ui.label(
            {"created": _("created"), "skipped": _("skipped"), "updated": _("updated")}[
                action
            ],
...
_metanamemap = util.sortdict(
    [(r"user", "User"), (r"date", "Date"), (r"node", "Node ID"), (r"parent", "Parent ")]
)
...
def _confirmbeforesend(repo, revs, oldmap):
    ...
    if ui.promptchoice(_("Send the above changes to %s (yn)?" "$$ &Yes $$ &No") % url):
        return False

    return True


_knownstatusnames = {"accepted", "needsreview", "needsrevision", "closed", "abandoned"}
...
def _tokenize(text):
    ...
    length = len(text)
    while pos < length:
        symbol = "".join(itertools.takewhile(lambda ch: ch not in special, view[pos:]))
        if symbol:
...
@command(
    "phabread", [("", "stack", False, _("read dependencies"))], _("DREVSPEC [OPTIONS]")
)
...
def phabupdate(ui, repo, spec, **opts):
    ...
    for i, drev in enumerate(drevs):
        ...
        if actions:
            params = {"objectIdentifier": drev[r"phid"], "transactions": actions}
