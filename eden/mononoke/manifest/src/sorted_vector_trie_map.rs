...
mod tests {
    ...
    async fn quickcheck_trie_map_ops(
        fb: FacebookInit,
        entries: SortedVectorMap<MPathElement, u8>,
    ) -> bool {
        ...
        check_eq!(
            trie_map.is_empty(),
            TrieMapOps::<(), _>::is_empty(&sv_trie_map)
        );

        let all_trie_map = trie_map
            .clone()
            .into_stream(&ctx, &())
            .await
            .unwrap()
            ...
            .await
            .unwrap();
        let all_sv_trie_map = sv_trie_map
            .clone()
            .into_stream(&ctx, &())
            .await
            .unwrap()
        ...
        while let Some((trie_map, sv_trie_map)) = expansions.pop() {
            let (sv_value, sv_subtries) = sv_trie_map.expand(&ctx, &()).await.unwrap();
