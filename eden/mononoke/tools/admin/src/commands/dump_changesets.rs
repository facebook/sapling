...
impl Format {
    ...
    async fn deserialize(self, data: &Bytes) -> Result<Vec<ChangesetEntry>> {
        match self {
            Self::Thrift => deserialize_cs_entries(data),
            Self::Plaintext => unimplemented!(),
        }
    }
}
    ...
    async fn fetch_extra_changesets(
        ...
        let start_commit = {
            if let Some(path) = self.start_from_file_end {
                load_last_commit(path.as_ref(), input_format).await?
...
pub async fn run(app: MononokeApp, args: CommandArgs) -> Result<()> {
    ...
    let input_format = Format::Thrift;
    ...
    let css = {
        let (mut file_css, db_css): (Vec<_>, Vec<_>) = future::try_join(
            stream::iter(
                args.merge_file
                    .iter()
                    .map(|path| load_file_contents(path.as_ref(), input_format))
...
async fn load_file_contents(filename: &Path, format: Format) -> Result<Vec<ChangesetEntry>> {
    let file_contents = Bytes::from(tokio::fs::read(filename).await?);
    format.deserialize(&file_contents).await
}

async fn load_last_commit(filename: &Path, format: Format) -> Result<Option<ChangesetId>> {
    Ok(load_file_contents(filename, format)
        .await?
        .last()
        .map(|e| e.cs_id))
}
