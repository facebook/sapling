...
#[derive(Args, Clone)]
pub struct RawBlobstoreFetchArgs {
    ...
    fn from_key_prefix(key: &str) -> Option<Self> {
            ...
            .chain(key.match_indices('.').map(|(index, _)| index + 1))
        {
            for (prefix, auto_decode_as) in [
                ...
                (
                    "hgaugmentedmanifest.",
                    DecodeAs::HgAugmentedManifestEnvelope,
                ),
...
async fn decode(key: &str, data: BlobstoreGetData, mut decode_as: DecodeAs) -> Decoded {
    ...
    match decode_as {
        ...
        DecodeAs::HgAugmentedManifestEnvelope => Decoded::try_debug(
...
pub async fn fetch(
    ...
    fetch_args: RawBlobstoreFetchArgs,
) -> Result<()> {
    let value = blobstore
    ...
    match value {
        ...
        Some(value) => {
            ...
            if let Some(output) = fetch_args.output {
                ...
                match decode(&fetch_args.key, value, fetch_args.decode_as).await {
