...
-- run the validator one more time, expect to fail and say it's because of filetypes
  $ REPOIDLARGE=0 validate_commit_sync 7 2>&1 | grep "Different filetype"
...
-- run the validator one more time, expect to fail and say it's because of contents
  $ REPOIDLARGE=0 validate_commit_sync 8 2>&1 | grep 'Different contents'
...
-- run the validator one more time, expect to fail and say it's because of contents
  $ REPOIDLARGE=0 validate_commit_sync 9 2>&1 | grep "is present in meg-mon"
...
-- run the validator one more time, expect to fail and say it's because of contents
  $ REPOIDLARGE=0 validate_commit_sync 10 2>&1 | grep "present in fbs-mon, but missing in meg-mon"
...
-- run the validator, check that commits are eqiuvalent
  $ REPOIDLARGE=0 validate_commit_sync 11 2>&1 | grep "Validated entry"
...
-- run the validator, check that commits are eqiuvalent
  $ REPOIDLARGE=0 validate_commit_sync 13 2>&1 | grep -E "(Preparing entry|Different contents)"
...
-- run the validator, check that commits are eqiuvalent
  $ REPOIDLARGE=0 validate_commit_sync 15 2>&1 | grep -E "(topological order|is not an ancestor)"
...
-- run the validator, check that commits are (1) validated (2) different
  $ REPOIDLARGE=0 validate_commit_sync 17 2>&1 | grep -E '(is a root|Validated entry)'
