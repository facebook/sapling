...
Check we get an error due to the missing aliases
  ...
  Walking edge types * (glob)
  Walking node types * (glob)
  Error: Could not step to OutgoingEdge { label: FileContentMetadataToGitSha1Alias, target: AliasContentMapping(AliasKey(GitSha1(GitSha1(96d80cd6c4e7158dbebd0849f4fb7ce513e5828c))))* (glob)
  * (glob)
  Caused by:
...
Check error as data fails if not in readonly-storage mode
  $ mononoke_new_walker --with-readonly-storage=false scrub --error-as-data-node-type AliasContentMapping -I deep -q -b master_bookmark 2>&1 | strip_glog
  Error: Error as data could mean internal state is invalid, run with --with-readonly-storage=true to ensure no risk of persisting it

Check counts with error-as-data-node-type
  $ mononoke_new_walker --scuba-log-file=scuba.json -l loaded scrub -q --error-as-data-node-type AliasContentMapping -I deep -b master_bookmark 2>&1 | strip_glog | sed -re 's/^(Could not step to).*/\1/' | uniq -c | sed 's/^ *//'
...
Check error-as-data-edge-type, should get an error on FileContentMetadataToGitSha1Alias as have not converted its errors to data
  $ mononoke_new_walker -l loaded scrub -q --error-as-data-node-type AliasContentMapping --error-as-data-edge-type FileContentMetadataToSha1Alias -I deep -b master_bookmark 2>&1 | strip_glog
...
Check error-as-data-edge-type, should get no errors as FileContentMetadataToGitSha1Alias has its errors converted to data
  $ mononoke_new_walker -l loaded scrub -q --error-as-data-node-type AliasContentMapping --error-as-data-edge-type FileContentMetadataToGitSha1Alias -I deep -b master_bookmark 2>&1 | strip_glog | sed -re 's/^(Could not step to).*/\1/' | uniq -c | sed 's/^ *//'
