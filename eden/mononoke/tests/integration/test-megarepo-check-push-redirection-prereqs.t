...
With COMPLEX mappping mulitiple paths are in wrong place (but the tool fails on first)
  $ EXPECTED_RC=1 quiet_grep "MPath" -- megarepo_tool_multirepo --source-repo-id $MEG_REPOID --target-repo-id $FBS_REPOID check-push-redirection-prereqs $MEG_C $FBS_C TEST_VERSION_NAME_COMPLEX
  * Execution error: MPath("*") exists in fbs-mon but not in meg-mon (glob)
  [1]
...
In version E one file is missing
  ...
  * Execution error: MPath("d/e") exists in fbs-mon but not in meg-mon (glob)
  [1]
...
In version F the file is present but the contents are wrong
  ...
  * Execution error: Found 1 files with different contents in meg-mon cs * (example: (MPath("d/e"), ContentId(Blake2(e6a553abb176f0acef0936e5d6e7930c8a590a62c07984bee8e3a8d5a2bb2ff9)), ContentId(Blake2(896ad5879a5df0403bfc93fc96507ad9c93b31b11f3d0fa05445da7918241e5d)))) (glob)
  [1]
...
Same for version C
  ...
  * all is well! (glob)
  ...
  $ EXPECTED_RC=1 quiet_grep "MPath" -- megarepo_tool_multirepo --source-repo-id $OVR_REPOID --target-repo-id $MEG_REPOID check-push-redirection-prereqs $OVR_C $MEG_G TEST_VERSION_NAME_COMPLEX
  * Execution error: MPath("*") exists in meg-mon but not in ovr-mon (glob)
  [1]
...
In version E one file is missing
  ...
  * Execution error: MPath("ma/b/c2/d/e") exists in meg-mon but not in ovr-mon (glob)
  [1]
...
In version F the file is present but the contents are wrong
  ...
  * Execution error: Found 1 files with different contents in ovr-mon cs * (example: (MPath("ma/b/c2/d/e"), ContentId(Blake2(e6a553abb176f0acef0936e5d6e7930c8a590a62c07984bee8e3a8d5a2bb2ff9)), ContentId(Blake2(896ad5879a5df0403bfc93fc96507ad9c93b31b11f3d0fa05445da7918241e5d)))) (glob)
  [1]
...
Version H has file vs directory conflict
  $ EXPECTED_RC=1 quiet_grep "MPath" -- megarepo_tool_multirepo --source-repo-id $OVR_REPOID --target-repo-id $MEG_REPOID check-push-redirection-prereqs $OVR_H $MEG_G TEST_VERSION_NAME_COMPLEX
  * Execution error: MPath("*") exists in meg-mon but not in ovr-mon (glob)
  [1]
