...
With COMPLEX mappping mulitiple paths are in wrong place (but the tool fails on first)
  ...
  Some(NonRootMPath("a/b/c1/f")) is present in fbs-mon, but not in meg-mon (under Some(NonRootMPath("ma/b/c1/f")))
  Some(NonRootMPath("a/b/c2/f")) is present in fbs-mon, but not in meg-mon (under Some(NonRootMPath("ma/b/c2/f")))
  Some(NonRootMPath("d/e")) is present in fbs-mon, but not in meg-mon (under Some(NonRootMPath("ma/b/c2/d/e")))
...
Version H has file vs directory conflict
  ...
  Some(NonRootMPath("ma/b")) is present in meg-mon, but not in fbs-mon (under Some(NonRootMPath("ma/b")))
  [1]
...
Same for version C
  ...
  * all is well! (glob)
  ...
  Some(NonRootMPath("ma/b/c1/f")) is present in meg-mon, but not in ovr-mon (under Some(NonRootMPath("a/b/c1/f")))
  Some(NonRootMPath("ma/b/c2/d/e")) is present in meg-mon, but not in ovr-mon (under Some(NonRootMPath("d/e")))
  Some(NonRootMPath("ma/b/c2/f")) is present in meg-mon, but not in ovr-mon (under Some(NonRootMPath("a/b/c2/f")))
