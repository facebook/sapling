...
Force pushrebase should fail, because it pushes to a shared bookmark
  ...
  $ sl push --to master_bookmark --force --pushvar NON_FAST_FORWARD=true >/dev/null
  pushing * (glob)
...
Non-shared bookmark should work
  $ sl push --to master_bookmark_non_fast_forward --force --create -q
...
Bookmark-only pushrebase (Create a new bookmark, do not push commits)
  $ cd "$TESTTMP/small-hg-client-1"
  $ sl push -r master_bookmark^ --to master_bookmark_2 --create 2>&1 | grep creating
