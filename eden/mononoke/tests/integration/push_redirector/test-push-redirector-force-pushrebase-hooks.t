...
to the small repo
  ...
  $ sl push -r . --to test_bookmark --create
...
blocked by deny_files
  ...
  $ sl push -r . --to test_bookmark --force 2>&1 | grep moving
...
Note that the node is from the small repo, even though the hook is in the large repo
  ...
  $ sl push -r . --to test_bookmark --force
...
changeset id.
  ...
  $ hg up -q master_bookmark
  $ sl push -r . --to test_bookmark --pushvar NON_FAST_FORWARD=true
