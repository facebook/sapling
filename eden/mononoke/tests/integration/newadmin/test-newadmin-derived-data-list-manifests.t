...
Skeleton manifests
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "a" -B main skeleton-manifests | sort
  a/b/
  a/foo.txt
Skeleton manifests from root
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "" -i "$B" skeleton-manifests | sort
  A
  B
  a/
  b/
Skeleton manifests (recursive)
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "a" -i "$B" skeleton-manifests --recursive | sort
  a/b/bar.txt
  a/foo.txt
Skeleton manifests from root (recursive)
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "" -B main skeleton-manifests --recursive | sort
  A
  B
  C
  a/b/bar.txt
  ...
  a/foo.txt
Fsnodes
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "a" -B main fsnodes | sort
...
Fsnodes from root path
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "" -i "$B" fsnodes | sort
...
Unodes
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "a" -B main unodes | sort
...
Unodes from root
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "" -i "$B" unodes | sort
...
Deleted manifests
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "a" -i "$B" deleted-manifests | sort
...
Deleted manifests from root
  $ with_stripped_logs mononoke_newadmin derived-data -R repo list-manifests -p "" -B "main" deleted-manifests | sort
  a/bar.txt/ DeletedManifestV2Id(Blake2(fa523e73a133223c61a827b226f8e339e136957ff48d7614d55dd0e18a42c19d))
