...
Check we can upload and fetch an arbitrary blob.
  ...
  $ mononoke_admin raw-blobstore --storage-name blobstore upload --key repo0000.somekey --value-file "$TESTTMP/value"
  Writing 6 bytes to blobstore key repo0000.somekey
  $ mononoke_admin raw-blobstore --storage-name blobstore fetch -q repo0000.somekey -o "$TESTTMP/fetched_value"
...
so we need to give the full key.
  ...
  $ mononoke_admin raw-blobstore --storage-name blobstore fetch -q repo0000.somekey -o "$TESTTMP/fetched_value_unlinked"
  No blob exists for repo0000.somekey

Examine some of the data
  $ mononoke_admin raw-blobstore --storage-name blobstore fetch repo0000.changeset.blake2.f9d662054cf779809fd1a55314f760dc7577eac63f1057162c1b8e56aa0f02a1
  Key: repo0000.changeset.blake2.f9d662054cf779809fd1a55314f760dc7577eac63f1057162c1b8e56aa0f02a1
  Ctime: * (glob)
  Size: 194
...
Test --blobstore-scrub-action works
  ...
  $ ls $TESTTMP/blobstore/1/blobs | wc -l
  34
  $ mononoke_admin raw-blobstore --storage-name blobstore fetch repo0000.content.blake2.6e07d9ecc025ae219c0ed4dead08757d8962ca7532daf5d89484cadc5aae99d8
