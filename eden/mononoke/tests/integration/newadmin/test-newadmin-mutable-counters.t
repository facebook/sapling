...
Validate if we can create new mutable counters
  $ mononoke_admin mutable-counters -R repo set foo 7
  Value of foo in repo repo(Id: 0) set to 7
  $ mononoke_admin mutable-counters -R repo set bar 9
  Value of bar in repo repo(Id: 0) set to 9

Validate if we can update an existing counter
  $ mononoke_admin mutable-counters -R repo set foo 10 --prev-value 7
...
Validate if all the new added counters are present
  ...
  bar                           =9
  foo                           =10
  $ mononoke_admin mutable-counters -R repo get bar
  Some(9)
  $ mononoke_admin mutable-counters -R repo get baz
  None
