...
returns a segment representing E, F and G
...
returns 4 segments (order can vary so only compare quantity)
...
returns 3 segments, the order of the last two can be switched so let's glob them
