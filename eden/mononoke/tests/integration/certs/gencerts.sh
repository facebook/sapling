...
openssl req -x509 -new -nodes -key root-ca.key -subj "/C=US/ST=CA/O=FakeRootCanal/CN=mononoke.com" -sha256 -days 365 -out root-ca.crt
...
if grep -q '\[ v3_ca \]' "$1"; then
  ...
  openssl x509 -extfile "$1" -extensions v3_ca -req -in localhost.csr -CA root-ca.crt -CAkey root-ca.key -CAcreateserial -out localhost.crt -days 365 -sha256
else
  openssl x509 -req -in localhost.csr -CA root-ca.crt -CAkey root-ca.key -CAcreateserial -out localhost.crt -days 365 -sha256
fi
