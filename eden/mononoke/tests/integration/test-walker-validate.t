...
validate, check route is logged on unexpected error (forced with chaos blob)
  $ mononoke_walker --blobstore-read-chaos-rate=1 --cachelib-only-blobstore -l validate validate -q -I deep -b master_bookmark --scuba-log-file scuba-error.json 2>&1 | strip_glog
  Performing check types [HgLinkNodePopulated]
...
validate, expecting validation fails
  $ mononoke_walker -l validate validate -q -I deep -b master_bookmark --scuba-log-file scuba.json 2>&1 | strip_glog
  Performing check types [HgLinkNodePopulated]
