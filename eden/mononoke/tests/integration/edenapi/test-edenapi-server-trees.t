...
Send a tree request by manifest id. With route-to-augmented on for this repo and the
augmented manifest derived at commit creation (mirroring the prod derived-data tailer),
the by-id request is transparently served the augmented manifest: byte-identical data
plus tree_aux_data + has_acl + children, even though the client did not request
augmented_trees.
  $ cat > keys << EOF
  > [
  >     ("", "$ROOT_MFID_1"),
...
The augmented manifest (and its AclManifest dependency) is derived at commit creation.
