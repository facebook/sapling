...
same run, but against metadata db
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p Changeset --chunk-size=2 --checkpoint-name=bonsai_deep_meta -I deep -i bonsai -i FileContent 2>&1 | strip_glog
  Repo bounds: (1, 4), repo: repo
  Starting chunk 1 with bounds (2, 4), repo: repo
  Seen,Loaded: 4,4, repo: repo
  Deferred: 1, repo: repo
...
run to a new checkpoint name, with checkpoint sampling set so that last chunk not included in checkpoint
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p Changeset --chunk-size=1 --checkpoint-sample-rate=2 --checkpoint-name=bonsai_deep2 --checkpoint-path=bonsai_deep2 -I deep -i bonsai -i FileContent 2>&1 | strip_glog
  Repo bounds: (1, 4), repo: repo
...
run again, should have no catchup, but main bounds will continue from checkpoint
  $ sleep 1
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p Changeset --chunk-size=1 --checkpoint-name=bonsai_deep2 --checkpoint-path=bonsai_deep2 -I deep -i bonsai -i FileContent 2>&1 | strip_glog
...
setup for both a catchup due to a new commit, plus continuation from a checkpoint.  First create the partial checkpoint by setting sample rate
 ...
 hg setup.  First create the partial checkpoint by setting sample rate
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p BonsaiHgMapping --chunk-size=1 --checkpoint-sample-rate=3 --checkpoint-name=hg_deep --checkpoint-path=test_sqlite -I deep -i hg -i FileContent 2>&1 | strip_glog
  Repo bounds: (1, 5), repo: repo
  Starting chunk 1 with bounds (4, 5), repo: repo
...
OldestFirst setup for both a catchup due to a new commit, plus continuation from a checkpoint.  First create the partial checkpoint by setting sample rate
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p Changeset -d OldestFirst --chunk-size=1 --checkpoint-sample-rate=3 --checkpoint-name=bonsai_deep3_oldest --checkpoint-path=test_sqlite -I deep -i bonsai -i FileContent 2>&1 | strip_glog
  Repo bounds: (1, 5), repo: repo
...
OldestFirst hg setup
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p BonsaiHgMapping -d OldestFirst --chunk-size=1 --checkpoint-name=hg_deep_oldest --checkpoint-path=test_sqlite -I deep -i bonsai -i FileContent 2>&1 | strip_glog
  Repo bounds: (1, 5), repo: repo
  Starting chunk 1 with bounds (1, 2), repo: repo
...
hg should have a run with both catchup and main bounds, and some deferred expected at end
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p BonsaiHgMapping --chunk-size=1 --checkpoint-sample-rate=3 --checkpoint-name=hg_deep --checkpoint-path=test_sqlite -I deep -i hg 2>&1 | strip_glog
  Found checkpoint with bounds: (2, 5), repo: repo
  Repo bounds: (1, 6), repo: repo
  Continuing from checkpoint run 1 chunk 3 with catchup Some((5, 6)) and main Some((1, 2)) bounds, repo: repo
  Starting chunk 4 with bounds (5, 6), repo: repo
...
OldestFirst, hg should have a run with only main bounds
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p BonsaiHgMapping -d OldestFirst --chunk-size=1 --checkpoint-name=hg_deep_oldest --checkpoint-path=test_sqlite -I deep -i hg 2>&1 | strip_glog
...
Check that the checkpoint low bound is not used if its too old
  $ sleep 2
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p Changeset --chunk-size=5 --state-max-age=1 --checkpoint-sample-rate=3 --checkpoint-name=bonsai_deep3 --checkpoint-path=test_sqlite -I deep -i bonsai -i FileContent 2>&1 | strip_glog
  Found checkpoint with bounds: (2, 5), repo: repo
  Repo bounds: (1, 6), repo: repo
...
OldestFirst, Check that the checkpoint high bound is not used if its too old
  $ mononoke_walker -L sizing -L graph scrub --checkpoint-version=v2 -q -p Changeset --chunk-size=5 --state-max-age=1 --checkpoint-sample-rate=3 --checkpoint-name=bonsai_deep3_oldest --checkpoint-path=test_sqlite -I deep -i bonsai -i FileContent 2>&1 | strip_glog
  Found checkpoint with bounds: (1, 4), repo: repo
  Repo bounds: (1, 6), repo: repo
  Checkpoint run 1 chunk 3 is too old at *s, running from repo bounds, repo: repo (glob)
  Starting chunk 1 with bounds (1, 6), repo: repo
