...
validate with LFS enabled, shallow
  $ mononoke_walker --scuba-log-file scuba-validate-shallow.json -L graph validate --include-check-type=FileContentIsLfs -I shallow -I BookmarkToBonsaiHgMapping -i hg -x HgFileNode -i FileContent -i FileContentMetadataV2 -q -b master_bookmark 2>&1 | strip_glog | grep -vE "(Bytes|Walked)/s"
  ...
  [INFO] [walker validate{repo=repo}] Seen,Loaded: 15,15
  [INFO] [walker validate{repo=repo}] Nodes,Pass,Fail:15,3,0; EdgesChecked:3; CheckType:Pass,Fail Total:3,0 FileContentIsLfs:3,0
...
validate with LFS enabled, deep.  Params are setup so that ValidateRoute contains the HgChangeset that originated a Bonsai and then the Bonsai points to the files it touched.
  ...
  [INFO] [walker validate{repo=repo}] Performing check types [FileContentIsLfs]
  [INFO] [walker validate{repo=repo}] Repo bounds: (1, 5)
  [INFO] [walker validate{repo=repo}] Starting chunk 1 with bounds (1, 5)
  [INFO] [walker validate{repo=repo}] Seen,Loaded: 37,37
  [INFO] [walker validate{repo=repo}] Nodes,Pass,Fail:37,4,0; EdgesChecked:4; CheckType:Pass,Fail Total:4,0 FileContentIsLfs:4,0
...
validate with LFS enabled, deep with simpler query.  Should have same output but touch less nodes to get there.
  ...
  [INFO] [walker validate{repo=repo}] Repo bounds: (1, 5)
  [INFO] [walker validate{repo=repo}] Starting chunk 1 with bounds (1, 5)
  [INFO] [walker validate{repo=repo}] Seen,Loaded: 20,20
  [INFO] [walker validate{repo=repo}] Nodes,Pass,Fail:20,4,0; EdgesChecked:4; CheckType:Pass,Fail Total:4,0 FileContentIsLfs:4,0
  [INFO] [walker validate{repo=repo}] Deferred: 0
  [INFO] [walker validate{repo=repo}] Completed in 1 chunks of size 100000
