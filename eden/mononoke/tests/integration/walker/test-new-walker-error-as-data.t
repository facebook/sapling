...
Check error as data fails if not in readonly-storage mode
  $ mononoke_walker --with-readonly-storage=false scrub --error-as-data-node-type AliasContentMapping -I deep -q -b master_bookmark 2>&1 | strip_glog | grep -vE "(Bytes|Walked)/s"
...
Check counts with error-as-data-node-type
  $ mononoke_walker --scuba-log-file scuba.json -l loaded scrub -q --error-as-data-node-type AliasContentMapping -I deep -b master_bookmark 2>&1 | strip_glog | grep -vE "(Bytes|Raw|Walked)/s" | sed -re 's/(Could not step to).*/\1/' | uniq -c | sed 's/^ *//'
...
Check error-as-data-edge-type, should get an error on FileContentMetadataV2ToGitSha1Alias as have not converted its errors to data
  $ mononoke_walker -l loaded scrub -q --error-as-data-node-type AliasContentMapping --error-as-data-edge-type FileContentMetadataV2ToSha1Alias -I deep -b master_bookmark 2>&1 | strip_glog | grep -vE "(Bytes|Walked)/s"
...
Check error-as-data-edge-type, should get no errors as FileContentMetadataV2ToGitSha1Alias has its errors converted to data
  $ mononoke_walker -l loaded scrub -q --error-as-data-node-type AliasContentMapping --error-as-data-edge-type FileContentMetadataV2ToGitSha1Alias -I deep -b master_bookmark 2>&1 | strip_glog | grep -vE "(Bytes|Raw|Walked)/s" | sed -re 's/(Could not step to).*/\1/' | uniq -c | sed 's/^ *//'
