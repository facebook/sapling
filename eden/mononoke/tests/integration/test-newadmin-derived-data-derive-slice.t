...
slice all ancestors of all bookmarks
  $ mononoke_newadmin derived-data -R repo slice -T test_sharded_manifests --slice-size 3 --all-bookmarks -o slices.json
  $ cat slices.json | jq .
  [
    {
      "slice_frontier": [
...
derive slice heads (C, G, K, and L)
  $ mononoke_newadmin derived-data -R repo derive-slice -T test_sharded_manifests -f slices.json --mode heads-only
...
derive the rest of the slices (A, B, D, E, F, G, I and J)
  $ mononoke_newadmin derived-data -R repo derive-slice -T test_sharded_manifests -f slices.json --mode excluding-heads
