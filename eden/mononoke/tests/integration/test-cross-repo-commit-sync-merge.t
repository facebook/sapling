...
run the sync, expected to fail, as parent of the synced commit is not present in the mapping
  $ mononoke_x_repo_sync 1 0 once --target-bookmark master_bookamrk --commit fbsource_master 2>&1 | grep -v "using repo"
...
run the sync again
  $ mononoke_x_repo_sync 1 0 once --target-bookmark bookmarktomerge --commit "$TOMERGE" 2>&1 | grep -v "using repo"
  * changeset resolved as: ChangesetId(Blake2(*)) (glob)
  ...
  * successful sync (glob)
  $ mononoke_x_repo_sync 1 0 once --target-bookmark master_bookmark --commit fbsource_master 2>&1 | grep -v "using repo"
  * changeset resolved as: ChangesetId(Blake2(*)) (glob)
  * Checking if * is already synced 1->0 (glob)
