...
async fn ensure_paths_exists(
    ...
    let mut paths_present = SelectAllPaths::query(
        read_conn,
    ...
    InsertPaths::query(write_conn, ctx.sql_query_telemetry(), &paths_to_insert[..])
        .await
...
async fn insert_filenodes(
    ...
    ctx.perf_counters()
        .increment_counter(PerfCounterType::SqlWrites);
    if replace {
        ReplaceFilenodes::query(write_conn, ctx.sql_query_telemetry(), &filenode_rows[..]).await?;
    } else {
        InsertFilenodes::query(write_conn, ctx.sql_query_telemetry(), &filenode_rows[..]).await?;
    }

    if !copydata_rows.is_empty() {
        ctx.perf_counters()
            .increment_counter(PerfCounterType::SqlWrites);
        InsertFixedcopyinfo::query(write_conn, ctx.sql_query_telemetry(), &copydata_rows[..])
            .await?;
    }
