...
The `CoreContext` carries session information used for audit logging but does not grant read exceptions to redaction. There is no mechanism to selectively grant read access to redacted content based on user identity.

For operational purposes, `RepoBlobstore` can create a log-only redaction view that retains the repository prefix and the inner blobstore's configured put behavior. When a repository sets `mirror_upload_redaction_bypass_enabled` in its config, the EdenAPI `upload/file` handler uses this view for the complete filestore operation if the caller holds the `mirror_upload` permission on the `mirror_commit_upload` tier ACL. When the config is false (the default), the handler skips the permission check and always enforces redaction. Redacted accesses through the log-only view remain audited but are not denied. Standard redaction key lists contain canonical `ContentId` keys, so this permits Modern Sync to reproduce redacted file content while normal destination reads and non-filestore writes retain redaction enforcement.
...
**Shadow Synchronization** - Modern Sync intentionally reads its source repository unredacted so the AWS shadow remains a complete backup. The AWS operational shadow repositories set `mirror_upload_redaction_bypass_enabled` in their config, and the Modern Sync job holds the `mirror_upload` permission, so the AWS server accepts its hash-validated canonical content uploads even when the destination already enforces that redaction. The redaction key lists must be synchronized to AWS first; uploads from callers without the `mirror_upload` permission and normal destination reads remain blocked.
...
- `sync-to-aws` - Sync every key list referenced by the active prod redaction config to AWS. It uses a running Mononoke-AWS pod when available, or a temporary Kubernetes job otherwise. For testing changes before the corresponding Mononoke image is deployed, `--bootstrap-monad` strips and uploads the local opt binary into an isolated temporary job; this transfers approximately 1.4 GB and should not be used for routine operation.
