...
2. Implementing the `run` method to perform validation and return a `HookExecution` (constructed with `HookExecution::accepted()` or `HookExecution::rejected(info)`). `HookExecution` is a struct wrapping a `HookResult` (`Accepted` or `Rejected`) plus any `extra_logs` the hook chose to emit for debugging.
