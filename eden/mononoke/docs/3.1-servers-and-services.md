...
**Location**: `servers/slapi/slapi_server/`
...
The server handles clone, pull, and push operations from Sapling clients, as well as on-demand file fetches from EdenFS instances. Request handling is implemented in the `repo_listener/` subdirectory, which manages TCP connections, TLS termination, and routing to protocol handlers. The actual protocol implementation resides in `servers/slapi/slapi_service/`, which translates HTTP requests into operations on the repository layer.
...
**Location**: `servers/git/git_server/`
**Binary**: `fbcode//eden/mononoke/servers/git/git_server:git_server`
...
**Location**: `servers/lfs/lfs_server/`
**Binary**: `fbcode//eden/mononoke/servers/lfs/lfs_server:lfs_server`
...
For operational details about specific servers, refer to component directories: `servers/slapi/`, `scs/`, `servers/git/`, `servers/lfs/`, `servers/land_service/`, and `derived_data/remote/`.
