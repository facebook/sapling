...
#[fbinit::test]
async fn test_add_sync_target_invalid_changesets_to_merge(fb: FacebookInit) -> Result<(), Error> {
    let ctx = CoreContext::test_mock(fb);
    let mut test = MegarepoTest::new(&ctx).await?;
    let target: Target = test.target("target".to_string());
    ...
    // Bookmark from second source doesn't exist - should fail
    let second_source_cs_id = CreateCommitContext::new_root(&ctx, &test.blobrepo)
        .add_file("second", "second")
        .commit()
        .await?;
    ...
    let one_more_second_source_cs_id =
        CreateCommitContext::new(&ctx, &test.blobrepo, vec![second_source_cs_id])
            .add_file("second", "second2")
            .commit()
            .await?;
