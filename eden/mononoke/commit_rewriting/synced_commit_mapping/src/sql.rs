...
pub async fn add_many_in_txn(
    ...
    let (txn, _result) =
        InsertMapping::query_with_transaction(txn, ctx.into(), &insert_entries).await?;
    ...
    let (txn, result) =
        InsertWorkingCopyEquivalence::query_with_transaction(txn, ctx.into(), &ref_entries).await?;
