...
async fn expand_all_git_submodule_file_changes<'a, R: Repo>(
    ...
    let (orig_fcs, mut expanded_fcs): (
        ...
        .try_fold(
            ...
            |(mut orig_fcs, mut exp_fcs), exp_fc| {
                match exp_fc {
                    ...
                    ExpansionFileChange::Generated((p, fc)) => {
                        ...
                        match exp_fcs.get(&path) {
                            ...
                            None => exp_fcs.insert(path, file_change),
                        };
                        Ok((orig_fcs, exp_fcs))
                    }
                }
...
async fn expand_git_submodule_file_change<'a, R: Repo>(
    ...
    let (copy_per_submodule, expanded_file_changes, _) = exp_results.into_iter().fold(
        ...
        move |(mut copy_per_submodule, mut acc_fcs, already_copied), (sm_path, fcs)| {
            ...
            acc_fcs.extend(fcs.into_iter().map(ExpansionFileChange::Generated));
...
async fn handle_submodule_deletion<'a, R: Repo>(
    ...
    if sm_exp_data.submodule_deps.contains_key(&deleted_path) {
        ...
        return Ok(generated_deletions
            .into_iter()
            .map(|p| ExpansionFileChange::Generated((p, FileChange::Deletion)))
            .collect());
    };
...
impl ExpansionFileChange {
    pub fn into_inner(self) -> (NonRootMPath, FileChange) {
        match self {
            Self::Original(path_fc) | Self::Generated(path_fc) => path_fc,
        }
    }
