...
#[async_trait]
impl Healer for SyncHealer {
    async fn heal(&self, ctx: &CoreContext, minimum_age: ChronoDuration) -> Result<HealResult> {
        let now = DateTime::now().into_chrono();
        let healing_deadline = DateTime::new(now - minimum_age);
        self.heal(ctx, healing_deadline)
            .await
            .map(|(processed_full_batch, processed_rows)| HealResult {
