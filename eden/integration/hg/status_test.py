...
class StatusTest(EdenHgTestCase):
    ...
    def test_scm_status_cache(self) -> None:
        ...
        with self.get_thrift_client_legacy() as client:
            ...
            # Journal seq advances and it include changes outside of .hg folder
            self.touch("world.txt")
            self.assert_status({"world.txt": "?"})  # this call misses the cache
            miss_cnt += 1
            self.counter_check(client, miss_cnt=miss_cnt, hit_cnt=hit_cnt)
            ...
            miss_cnt += 1
            self.assert_status({"world.txt": "A"})  # this call hits the cache
            hit_cnt += 1
            self.counter_check(client, miss_cnt=miss_cnt, hit_cnt=hit_cnt)
            ...
            # testing customized calls with different parameters
            ...
            # so the cacued result from the previous assert_status call is lost
            # at this point
