...
class AutoStopTest(EdenTestCase):
    async def update_validity_interval(self, interval: str) -> None:
        config_text = f"""
[config]
[core]
...
"""
    ...
    async def _run_test(
        self, invalidate_fn: Callable[[], None], timeout: float = 15
    ) -> None:
        await self.update_validity_interval("20ms")
    ...
    async def test_delete_lock_file(self) -> None:
        def delete_lock_file() -> None:
            (self.eden.eden_dir / "lock").unlink()

        await self._run_test(delete_lock_file)

    async def test_replace_lock_file(self) -> None:
        ...
        await self._run_test(replace_lock_file)

    async def test_move_state_dir(self) -> None:
        def move_state_dir() -> None:
            new_path = Path(self.tmp_dir) / "new-eden-dir"
            self.eden.eden_dir.rename(new_path)

        await self._run_test(move_state_dir)

    async def test_runs_normally(self) -> None:
        ...
        with self.assertRaises(TimeoutError):
            await self._run_test(noop, timeout=5)
