...
def debugsvnlog(ui, **opts):
    ...
    """
    if svn is None:
        raise error.Abort(_("debugsvnlog could not load Subversion python " "bindings"))
...
protomap = {"http": httpcheck, "https": httpcheck, "file": filecheck}
...
class svn_source(converter_source):
    def __init__(self, ui, repotype, url, revs=None):
        ...
        if not (
            ...
            or (os.path.exists(url) and os.path.exists(os.path.join(url, ".svn")))
            or issvnurl(ui, url)
        ):
        ...
        try:
            ...
            raise MissingTool(
                _("Subversion python bindings are too old, 1.4 " "or later required")
            )
        ...
        if revs:
            ...
            try:
                ...
                raise error.Abort(_("svn: revision %s is not an integer") % revs[0])
    ...
    def exists(self, path, optrev):
        try:
            svn.client.ls(
                self.url.rstrip("/") + "/" + quote(path), optrev, False, self.ctx
            )
            return True
        except svn.core.SubversionException:
            return False
...
class svn_sink(converter_sink, commandline):
    ...
    def add_dirs(self, files):
        add_dirs = [d for d in sorted(self.dirs_of(files)) if d not in self.manifest]
        if add_dirs:
