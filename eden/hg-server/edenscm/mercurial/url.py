    ...
    def find_user_password(self, realm, authuri):
        ...
        if not user or not passwd:
            ...
            if not self.ui.interactive():
                raise error.Abort(
                    _("http authorization required for %s") % util.hidepassword(str(u))
                )
...
def _generic_proxytunnel(self):
    proxyheaders = dict(
        [(x, self.headers[x]) for x in self.headers if x.lower().startswith("proxy-")]
    )
...
if has_https:
    ...
    class httpsconnection(httplib.HTTPConnection):
        ...
        def __init__(
            self, host, port=None, key_file=None, cert_file=None, *args, **kwargs
        ):
...
class httpbasicauthhandler(urlreq.httpbasicauthhandler):
    ...
    def retry_http_basic_auth(self, host, req, realm):
        user, pw = self.passwd.find_user_password(realm, urllibcompat.getfullurl(req))
        if pw is not None:
            raw = "%s:%s" % (user, pw)
...
def opener(ui, authinfo=None, useragent=None):
    ...
    if ui.configbool("ui", "usehttp2"):
        handlers = [
            httpconnectionmod.http2handler(ui, passwordmgr(ui, ui.httppasswordmgrdb))
        ]
    else:
    ...
    handlers.extend((httpbasicauthhandler(passmgr), httpdigestauthhandler(passmgr)))
