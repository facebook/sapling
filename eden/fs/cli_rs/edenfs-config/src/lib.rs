...
mod tests {
    ...
    #[test]
    fn test_unknown_telemetry_settings_are_preserved_and_merged() {
        let mut loader = EdenFsConfig::loader();
        loader.load(
            toml::from_str(
                "[telemetry]\nscribe-cat = 'original'\n[telemetry.extra]\nfirst = true\n",
        ...
        assert_eq!(
            serialized["telemetry"]["scribe-cat"].as_str(),
            Some("override")
        );
        assert_eq!(
            serialized["telemetry"]["extra"]["first"].as_bool(),
            Some(true)
        );
        assert_eq!(
            serialized["telemetry"]["extra"]["second"].as_bool(),
            Some(true)
        );
    }
}
