...
DirList TreeInode::readdir(DirList&& list, off_t off) {
...

  {
    ...
    entries.emplace_back(".", dtype_t::Dir, getNodeId());
    ...
    auto parent_inode = parent ? parent->getNodeId() : getNodeId();
  ...
  while (entry_iter < entries.end()) {
    ...
    if (entry.ino.hasValue()) {
      ...
      // this function.
      st.st_ino = getChildInodeNumber(PathComponentPiece{entry.name}).get();
    }
