...
void eden_get_entry_information(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    auto result =
        client->semifuture_getEntryInformation(mountPoint, filePaths, sync)
            .via(eventBase)
            .get();
    ...
    for (const auto& entry : result) {
      if (entry.getType() == EntryInformationOrError::Type::error) {
        ++totalErrors;
      }
    }
...
BENCHMARK(eden_get_entry_information)
    ->UseManualTime()
