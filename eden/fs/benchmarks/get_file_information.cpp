...
void eden_get_file_information(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    auto result =
        client->semifuture_getFileInformation(mountPoint, filePaths, sync)
            .via(eventBase)
            .get();
    ...
    for (const auto& entry : result) {
      if (entry.getType() == FileInformationOrError::Type::error) {
        ++totalErrors;
      }
    }
...
BENCHMARK(eden_get_file_information)
    ->UseManualTime()
